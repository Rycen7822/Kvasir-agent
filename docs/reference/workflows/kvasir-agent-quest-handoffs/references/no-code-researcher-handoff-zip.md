# No-code researcher handoff zip for research/Kvasir-agent quests
...
- selected Kvasir-agent artifact records for provenance.
...
- `.ka/` runtime logs and the full `.git/` repository;
...
## Kvasir-agent artifact note

For document/package maintenance, prefer recording a `milestone` rather than a generic `report`. In one Quest 001 session, `ka_artifact_record(kind="report")` semantically matched an old report artifact and did not create a clear new record, while `kind="milestone"` produced the intended durable checkpoint.
