...
> Codex adapter note: this stage skill is bundled for Kvasir-agent. Prefer the stable curated MCP `ka_*` tools for repeated Kvasir-agent state/status/context workflows. Load this support skill only when its stage is relevant. Runtime state lives under `<project>/Kvasir-agent/`.
...
- a main-experiment summary image sent to a removed connector
...
Kvasir-agent figures should feel academic, restrained, and clear.
...
- `removed connector_milestone`
  - quick summary image for external messaging / chat / copilot milestone reporting
  - usually `png`
...
- `assets/kvasir-agent-academic.mplstyle`
...
- `plt.style.use(<path-to-assets/kvasir-agent-academic.mplstyle>)`
...
- small removed connector summary: around `5.2 x 3.2 in`
