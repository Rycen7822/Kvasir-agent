...
```bash
python scripts/verifier.py --input examples/input_dois.txt --out examples/carcs.jsonl
```
...
默认从本 skill 的 `paper_ranking/` 读取会议分级和期刊分区快照，目录应包含 `conference_ranking.csv` 和 `journal_ranking.csv`。

如需使用外部更新的快照，可以设置 `PAPER_RANKING_DIR`：
