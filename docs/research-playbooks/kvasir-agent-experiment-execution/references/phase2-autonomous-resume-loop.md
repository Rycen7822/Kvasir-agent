...
Use this note when a Kvasir-agent Quest asks for autonomous, experiment-only continuation of a resumable Phase 2 `main_100m` run until formal `Y_recover` / gate satisfaction.
...
2. Use Kvasir-agent-native execution for quest commands. Prefer `ka_bash_exec` for shell/Python/monitoring so bash sessions and logs are durable; avoid generic terminal/process background jobs for Kvasir-agent quest work unless no native background facility exists, and then immediately write a quest-local summary file and record the exception.
