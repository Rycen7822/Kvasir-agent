...
Use this note when a Kvasir-agent Phase 2 `main_100m`/full-budget runner has moved from a single bounded corpus sample to repeated resume chunks and needs to prove that later invocations advance through the streaming corpus instead of rereading the same local split/sample rows.
