...
Use this note when a Kvasir-agent quest's experiment docs have become long or fragmented and the user asks for conservative cleanup/maintenance while experiments are still active.

## Pattern

1. Stay inside the quest scope. Reconfirm the active quest root and any explicit no-touch paths before editing. For Quest001-style work, do not touch unrelated development checkouts unless the user explicitly asks.
