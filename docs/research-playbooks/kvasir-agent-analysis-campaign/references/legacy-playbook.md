...
---
name: kvasir-agent-analysis-campaign
...
This is the shared Kvasir-agent protocol for supplementary experiments after a durable result.
...
- Codex-native execution boundary: use Codex-native file/shell/Git/test/build tools for routine inspection, setup checks, and non-evidence smoke tests. Use `ka_bash_exec` for formal campaign slice commands or validations whose logs must become Kvasir-agent quest evidence.
- Prefer `ka_bash_exec` for campaign slice commands so each run has a durable session id, quest-local log folder, and later `read/list/kill` control.
- Keep ordinary subtask completions concise. When an analysis campaign or a stage-significant campaign checkpoint is complete, upgrade to a richer `ka_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` report.
...
It preserves the core old Kvasir-agent analysis-experimenter discipline:
...
8. Before returning to `write`, outline selection, or paper bundle submission, use the Codex-native `ka_get_analysis_campaign` tool if available and verify there is no active campaign with pending required slices.
...
If ids, active refs, or current quest state are unclear after restart, call `artifact.get_quest_state(detail='summary')` and `ka_artifact_record evidence path resolution(...)` before launching or recording slices.
...
- `ka_bash_exec` session ids and managed shell logs for campaign runs
...
After the charter and launch decision are durably recorded, send one threaded `ka_artifact_record payload={'kind': 'milestone'}, ...)` update naming:
...
If ids or refs are unclear, recover them first with `ka_artifact_record evidence path resolution(...)`, `artifact.get_analysis_campaign(...)`, or `artifact.list_paper_outlines(...)` instead of guessing.
...
After each slice finishes, call `ka_artifact_record_analysis_slice(...)` immediately so the result is mirrored back to the parent branch and the next slice can be activated.
If a slice fails or becomes infeasible, still call `ka_artifact_record_analysis_slice(...)` with an honest non-success status plus the real blocker and next recommendation; do not leave the campaign state ambiguous.
...
Each `ka_artifact_record_analysis_slice(...)` call should also include an `evaluation_summary` with exactly these six fields:
...
- once the smoke test passes, launch the real slice with `ka_bash_exec mode='detach', ...)` and normally leave `timeout_seconds` unset for that long run
...
- if you need a middle section that was omitted from that default preview, use `ka_bash_exec mode='read', id=..., start=..., tail=...)`
- monitor them with `ka_bash_exec mode='list')` and `ka_bash_exec mode='read', id=..., tail_limit=..., order='desc')`
- after the first read, prefer `ka_bash_exec mode='read', id=..., after_seq=last_seen_seq, tail_limit=..., order='asc')` for incremental monitoring
- if ids become unclear, recover them through `ka_bash_exec mode='history')`
- launch long slices with a structured `comment` such as `{stage, goal, action, expected_signal, next_check}`
- use `silent_seconds`, `progress_age_seconds`, `signal_age_seconds`, and `watchdog_overdue` from `ka_bash_exec mode='list'|'read', ...)` as the default stall checks
...
- after the first meaningful signal and then at real checkpoints (e.g., completion, blocker, recovery, or a materially changed evidence frontier), send `ka_artifact_record payload={'kind': 'progress'}, ...)` so the user sees the newest real state
- after each completed sleep / await monitoring cycle for an active slice, inspect state first; only send another `ka_artifact_record payload={'kind': 'progress'}, ...)` update if the user-visible state materially changed
- include the estimated next reply time or next check time in those monitoring updates
- stop them with `ka_bash_exec mode='kill', id=..., wait=true, timeout_seconds=...)` if the slice is invalid, wedged, or superseded; add `force=true` when immediate termination is required
- when you control the slice code, prefer a throttled `tqdm` progress reporter and, when feasible, pair it with concise `__KA_PROGRESS__` lines carrying phase and ETA
...
- working surface:
  - `.ka/worktrees/<slice_id>/` when isolated worktrees are used
- experiment surface:
...
Before a long slice starts, emit a `progress` artifact or `ka_artifact_record payload={'kind': 'progress'}, ...)` update so the quest shows that the slice is active.
...
Send a threaded `ka_artifact_record payload={'kind': 'milestone'}, ...)` update at that point with the new boundary or risk.
...
When the aggregated campaign report is complete, send a richer threaded `ka_artifact_record payload={'kind': 'milestone'}, ...)` update.
