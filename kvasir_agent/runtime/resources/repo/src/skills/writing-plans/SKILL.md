...
1. Load the relevant Kvasir-agent stage skill and follow the Codex-native operation boundary: use Codex-native tools for routine shell/Git/Python checks, and use `ka_bash_exec` only for formal evidence commands that need quest-local provenance. Use `ka_artifact_record` and native file editing for durable outputs.
