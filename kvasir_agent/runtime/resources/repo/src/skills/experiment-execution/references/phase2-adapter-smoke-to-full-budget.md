...
Use this note when a Kvasir-agent quest has progressed from Phase 2 selector/corpus provenance and a planned retrofit recorder to a real but bounded recurrent-core adapter smoke run. It captures how to keep advancing E10 without overclaiming G2.
...
7. Record a milestone and decision after validation. If `ka_record_main_experiment` fails on optional chart rendering, use validated disk artifacts plus `ka_artifact_record` for durable routing rather than rerunning the experiment blindly.
