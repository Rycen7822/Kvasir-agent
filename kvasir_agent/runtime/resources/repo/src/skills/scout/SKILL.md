...
- **Use Codex-native execution for routine shell/CLI/Python/node/git/npm/uv/repo-inspection checks. Use `ka_bash_exec` only when the inspection command itself must become Kvasir-agent quest evidence.**
...
- **If scouting only needs durable quest context, prefer `artifact.read_quest_documents(...)`, `artifact.get_quest_state(...)` and native file reads instead of shelling out.**
