...
- Codex-native execution boundary: use Codex-native tools for routine file inspection, Git mechanics, environment checks, non-evidence tests, and code edits. Use `ka_bash_exec` for formal experiment commands, real runs, baseline/reproduction/ablation evidence, or paper-facing validations whose logs must become Kvasir-agent quest evidence.
- Prefer `ka_bash_exec` for experiment commands so each run gets a durable session id, quest-local log folder, and later `read/list/kill` control.
...
- **Use Codex-native execution for routine shell/CLI/Python/node/git/npm/uv/environment checks and non-evidence tests. Use `ka_bash_exec` for formal experiment runs, baseline/reproduction/ablation evidence, and paper-facing validations that require quest-local provenance.**
