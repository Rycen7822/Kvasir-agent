...
- Codex-native execution boundary: use Codex-native tools for routine file inspection, Git mechanics, dependency checks, smoke checks, quick validations, tests, and process management. Use `ka_bash_exec` for optimization runs whose logs must become Kvasir-agent research evidence, especially formal evaluations, long runs, or frontier-shifting candidate evidence.
...
- Use Codex-native execution for routine smoke checks and quick validations; use `ka_bash_exec` for long or formal optimization runs that need durable project-local provenance.
