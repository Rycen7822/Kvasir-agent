...
description: Create durable Kvasir-agent project handoff documents, AGENTS.md files, researcher packages, and verified sync artifacts.
version: 1.0.1
metadata:
  hermes:
    category: research
    tags: [handoff, kvasiragent, research, agents, synchronization, verification]
...
> When a handoff becomes durable, record it with `ka_artifact_record` as a milestone/report and include the absolute research root path in the payload.

# Research Quest Handoffs

Use this skill when the user asks for onboarding notes, a guide for another agent, an `AGENTS.md`, a quick-start document, or a synced handoff for a research/Kvasir-agent project.
...
## Kvasir-agent workflow

1. Read research state with Kvasir-agent tools when available.
...
4. If a second project copy exists, sync the intended file or research subtree explicitly unless the latest user instruction limits the work to the current research state/root only; in that case, state the no-sync constraint in the handoff and do not touch the secondary copy.
...
- Do not overwrite target-local project configuration while syncing, such as `Kvasir-agent/config/codex-native.yaml`, unless the user explicitly asks.
...
When the user asks to send a quest to another researcher for analysis, create a compact no-code zip with a root `README_FOR_RESEARCHER.md`, manifest files, active idea docs, experiment plans, result reports, figures/tables, and small source-of-truth evidence artifacts. For external researcher analysis, default to a minimal tier (idea/protocol/status/report/final evidence only) and expand only on request; avoid bundling long command ledgers, full figure directories, or broad provenance if the user says the package is too cluttered. Exclude code, model weights/checkpoints, raw caches, PDFs unless requested, `.ka/`, and secondary workspace copies under current no-sync constraints. See `references/no-code-researcher-handoff-zip.md`.
...
See `references/kvasir-agent-quest-handoff-sync.md` for the session-derived checklist and verification pattern.
