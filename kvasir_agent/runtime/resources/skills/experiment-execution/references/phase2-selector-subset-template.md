...
Use this reference when a Kvasir-agent experiment command document advances from Phase 1B stats into Phase 2 selector subset setup, but real Phase 1A frozen benchmark results do not yet exist.
...
- `ka_artifact_record` requires `quest_id` as a top-level argument even if `payload.quest_id` is present.
