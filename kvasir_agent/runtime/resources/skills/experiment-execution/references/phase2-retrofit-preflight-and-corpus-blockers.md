...
Use this reference when a Kvasir-agent formal experiment has moved past Phase 2 selector setup and the next boundary is a real retrofit runner, but the full paper-scale training/evaluation chain may still be unavailable.
...
4. Run the command through `ka_bash_exec`, validate artifacts from disk, then record milestone/decision with `ka_artifact_record`.
