...
Use when a Kvasir-agent quest-local quickstart file (for example `AGENTS.md`) contains stale state that conflicts with `ka_get_quest_state`, formal experiment command documents, or recently recorded decisions.
...
- CFC / ETD-score / RR-score / RawRepeat / Middle-cycle were clarified as experiment-internal comparators/controls, not formal Kvasir-agent baseline artifacts.
