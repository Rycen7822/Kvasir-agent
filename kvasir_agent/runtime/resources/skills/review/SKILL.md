...
- Codex-native execution boundary: use Codex-native file/shell/Git/test/build tools for routine document builds, scripted checks, Git inspection, and file inspection. Use public MCP `ka_bash_exec` only when the command itself must become research evidence; ordinary Codex file tools are preferred for direct file IO.
