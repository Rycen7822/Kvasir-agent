...
- **Use `decision` to judge the route, not as an excuse to bypass the `ka_bash_exec ...)` / `project-local git state via backend service or Codex-controlled git action(...)` tool contract.**
