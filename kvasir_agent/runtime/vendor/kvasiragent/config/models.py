...
def default_config(home: Path) -> dict:
    return {
        ...
        "cloud": {
            "enabled": False,
            "base_url": "https://codexscientist.cc",
            "token": None,
            "token_env": "KVASIR_AGENT_TOKEN",
            "verify_token_on_start": False,
            "sync_mode": "disabled",
        },
        "acp": {
            "compatibility_profile": "kvasir-agent-acp-compat/v1",
...
def default_runners() -> dict:
    return {
        "codex": {
            ...
            # Increase native kactl tool timeout so codex can wait for long `ka_bash_exec` await/read operations
            # or other durable native tool calls without prematurely timing out.
            # Mirrors the upstream project's `codex.native_tool_tool_timeout_sec` default.
