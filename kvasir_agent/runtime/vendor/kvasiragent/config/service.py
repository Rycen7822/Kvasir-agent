...
This page edits `~/Kvasir-agent/config/connectors.yaml` directly.
...
- scan the QR code first so Kvasir-agent can persist `bot_token` and `account_id`
...
- Lingzhu is hosted directly by Kvasir-agent on `/metis/agent/api`
- keep `transport: openclaw_sse`
- use the same public Kvasir-agent origin and port that the browser is already serving on
- save once so Kvasir-agent can persist `auth_ak`; Rokid must use the same Bearer token
...
- once Codex answers correctly, Kvasir-agent flips this flag to `true`
- if the probe fails, Kvasir-agent writes the failure summary back into config and blocks startup
...
- Kvasir-agent hard-limits one turn to at most `5` total attempts, even if the config says more
...
"""
        ...
        for name, raw_config in payload.items():
            ...
            if name == "telegram":
                ...
                if not str(config.get("local_host") or "").strip():
                    warnings.append("lingzhu: `local_host` is empty; Kvasir-agent will fall back to `127.0.0.1`.")
    ...
    def _test_single_connector(self, name: str, config: dict, *, live: bool, delivery_target: dict[str, object] | None = None) -> dict:
        ...
        if delivery_target and name != "lingzhu":
            ...
            if not delivery_requested:
                ...
                if name == "qq":
                    warnings.append(
                        "QQ readiness is healthy, but no OpenID has been learned yet. Save credentials, then send one private QQ message so Kvasir-agent can auto-detect and save the `openid`."
                    )
                else:
    ...
    def _default_connector_probe_message(self, connector_name: str) -> str:
        ...
        if locale.startswith("zh"):
            return f"老师您好，这是一条来自 Kvasir-agent 设置页的 {connector_name} 连接测试消息。若您收到这条消息，说明当前绑定与发送链路已经打通。"
        return (
            f"Hello — this is a Kvasir-agent {connector_name} settings test message. "
    ...
    def _prepare_codex_probe_home(
        ...
        *,
        ...
        temp_home = tempfile.TemporaryDirectory(prefix="ka-codex-probe-")
    ...
    def _codex_missing_binary_guidance(self, config: dict) -> list[str]:
        ...
        if profile:
            guidance.extend(
                [
                    f"Then verify `codex --profile {profile}` works from a terminal before starting Kvasir-agent.",
                    "If that profile uses a custom provider, make sure its API key and Base URL are configured in Codex first.",
                ]
            )
        else:
            guidance.append("Run `codex login` (or just `codex`) once and finish authentication before starting Kvasir-agent.")
        guidance.append(
            "If you use a custom Codex path, set `runners.codex.binary` to its absolute path."
        )
        return guidance
    ...
    def _provider_profile_probe_hints(metadata: dict[str, object]) -> list[str]:
        ...
        if "coding.dashscope.aliyuncs.com" not in base_url:
            return [
                "Alibaba Bailian's generic DashScope / Qwen platform API is not supported by the Codex-backed Kvasir-agent path.",
    ...
    def _local_provider_probe_hints(metadata: dict[str, object]) -> list[str]:
        ...
        if requires_openai_auth is not False:
            hints.insert(
                0,
                "For local or self-hosted providers, add `requires_openai_auth = false` so Kvasir-agent can remove conflicting `OPENAI_*` auth variables.",
            )
        if not wire_api:
    ...
    def _missing_provider_env_guidance(
        *,
        ...
        guidance = [
            f"Set `runners.codex.env.{env_key}` in `~/Kvasir-agent/config/runners.yaml`, or export `{env_key}` before starting Kvasir-agent.",
        ]
        if provider_base_url_looks_local(str(metadata.get("base_url") or "").strip()):
            ...
            if metadata.get("requires_openai_auth") is not False:
                guidance.append(
                    "Also add `requires_openai_auth = false` to that local provider profile so Kvasir-agent can remove conflicting `OPENAI_*` auth variables."
                )
        guidance.append(
            f"Before retrying Kvasir-agent, run a real request such as `codex exec --profile {profile} --json --cd /tmp --skip-git-repo-check -` and verify it returns `HELLO`."
        )
        return guidance
    ...
    def _chat_wire_probe_version_block(
        ...
        *,
        ...
        errors = [
            "This provider uses `wire_api = \"chat\"`, but Kvasir-agent only probes chat-mode providers with `codex-cli 0.57.0`.",
        ]
        if detected_text:
            errors.append(f"Detected Codex CLI version: `{detected_text}`.")
        else:
            errors.append("Kvasir-agent could not determine the active Codex CLI version from the configured binary.")
        guidance = [
            "Install `npm install -g @openai/codex@0.57.0`, or set `runners.codex.binary` to a dedicated `0.57.0` binary.",
        ...
        if profile:
            ...
            return (
                ...
                [
                    ...
                    "If the provider expects the model from the Codex profile itself, set `model: inherit` in `~/Kvasir-agent/config/runners.yaml`.",
                    *provider_hints,
                    *local_hints,
                    "Then rerun diagnostics and start Kvasir-agent again.",
                ],
            )
        return (
            [
                "Run `codex login` (or just `codex`) once and complete login before starting Kvasir-agent.",
            ],
            [
                ...
                "If the configured model is not available to your Codex account, update `~/Kvasir-agent/config/runners.yaml` and try again.",
                "Then rerun diagnostics and start Kvasir-agent again.",
            ],
        )
    ...
    def _probe_codex_runner(self, config: dict) -> dict:
        ...
        if not resolved_binary:
            return {
                ...
                "errors": [
                    ...
                    "Kvasir-agent could not resolve the bundled or configured `codex` CLI.",
                ],
                "details": details,
        ...
        if profile and effective_model == "inherit" and not self._codex_should_inherit_model(requested_model):
            compatibility_warnings.append(
                f"Codex profile `{profile}` is provider-backed. Kvasir-agent is probing it with `model: inherit`."
            )
            ...
            return {
                ...
                "guidance": [
                    *self._codex_probe_failure_guidance(config)[1],
                    ...
                    "Confirm the configured model is available to your Codex setup. Kvasir-agent currently probes Codex with the configured runner model first.",
                ],
            }
            ...
            if fallback_timeout is None and fallback_result is not None:
                ...
                if fallback_ok:
                    ...
                    fallback_warning = (
                        ...
                        "Kvasir-agent fell back to the current Codex default model."
                    )
                    return {
                        "ok": True,
                        ...
                        "details": details,
                        "guidance": [
                            "Kvasir-agent switched the Codex runner model to `inherit` so future runs keep using the current Codex default model.",
                        ],
                    }
            else:
        ...
        if not ok:
            ...
            if details.get("model_fallback_attempted") and not details.get("model_fallback_used"):
                warnings.append("Kvasir-agent also tried the current Codex default model, but that fallback probe did not succeed.")
        ...
        if not ok and missing_env_key and profile:
            errors.append(
                f"Codex profile `{profile}` requires environment variable `{missing_env_key}`, but Kvasir-agent did not receive it."
            )
            failure_guidance = [
