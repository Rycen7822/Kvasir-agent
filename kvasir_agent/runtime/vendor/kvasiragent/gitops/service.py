...
def _ensure_local_git_identity(repo: Path) -> None:
    ...
    if not name:
        run_command(["git", "config", "--local", "user.name", "Kvasir-agent Agent"], cwd=repo, check=False)
    if not email:
        run_command(["git", "config", "--local", "user.email", "kvasiragent@example.invalid"], cwd=repo, check=False)
...
def canonical_worktree_root(repo: Path, run_id: str) -> Path:
    return repo / ".ka" / "worktrees" / slugify(run_id, "run")
