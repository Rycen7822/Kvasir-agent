...
# Hermes-headless backend: social connector packages are excluded from the
# packaged runtime. Keep inert compatibility helpers so legacy service methods
# can fail closed without importing connector modules.
def register_builtin_connector_bridges() -> None:
    return None


def register_builtin_channels(*args: object, **kwargs: object) -> None:
    return None


def get_channel_factory(name: str):
    raise RuntimeError("Kvasir-agent connector channels are disabled in Hermes-headless backend.")


def normalize_conversation_id(value: object) -> str:
    return str(value or "local:default").strip() or "local:default"


def conversation_identity_key(value: object) -> str:
    return normalize_conversation_id(value).lower()
...
class ArtifactService:
    ...
    def _analysis_manifest_path(self, quest_root: Path, campaign_id: str) -> Path:
        return ensure_dir(quest_root / ".ka" / "analysis_campaigns") / f"{campaign_id}.json"
    ...
    def _baseline_workspace_roots(self, quest_root: Path) -> list[Path]:
        ...
        research_state = read_json(quest_root / ".ka" / "research_state.json", {})
        ...
        *,
        ...
        if not fetched.get("ok"):
            ...
            return {
                ...
                "message": "Metadata is temporarily unavailable. Open the arXiv link directly while Kvasir-agent retries later.",
        ...
        *,
        ...
        if completion_request is None:
            return {
                ...
                "quest_id": snapshot.get("quest_id"),
                "message": (
                    "Quest completion requires explicit user approval first. "
                    "Ask the user through Hermes and record the decision with a curated ka_* tool; "
                    f"decision_type={QUEST_COMPLETION_DECISION_TYPE}."
                ),
            }
    ...
    def _recent_inbound_messages(self, quest_root: Path, *, limit: int) -> list[dict]:
        conversation_path = quest_root / ".ka" / "conversations" / "main.jsonl"
    ...
    def _interaction_state_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "interaction_state.json"
