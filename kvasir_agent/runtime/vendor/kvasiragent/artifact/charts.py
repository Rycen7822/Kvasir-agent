...
def _draw_brand_lockup(draw: ImageDraw.ImageDraw, image: Image.Image, *, width: int) -> None:
    ...
    text_bbox = draw.textbbox((0, 0), "Kvasir-agent", font=brand_font)
    ...
    draw.text((text_left, 46), "Kvasir-agent", fill=MORANDI_TEXT, font=brand_font)
...
def _load_brand_mark() -> Image.Image | None:
    candidates = [
        ...
        Path(__file__).resolve().parents[3] / "assets" / "branding" / "kvasir-agent-mark.png",
    ]
