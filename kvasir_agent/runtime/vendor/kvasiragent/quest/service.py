...
from ..conversation_identity import conversation_identity_key, normalize_conversation_id, parse_conversation_id
...
class QuestService:
    ...
    def _binding_sources_payload(self, quest_root: Path) -> dict[str, list[str]]:
        bindings_path = quest_root / ".ka" / "bindings.json"
    ...
    def _runtime_state_lock_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "runtime_state.lock"
    ...
    def _research_state_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "research_state.json"

    @staticmethod
    def _lab_canvas_state_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "lab_canvas_state.json"
    ...
    def workspace_roots(self, quest_root: Path) -> list[Path]:
        ...
        worktrees_root = quest_root / ".ka" / "worktrees"
    ...
    def _artifact_projection_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "cache" / "artifact_projection.v2.json"

    @staticmethod
    def _artifact_projection_lock_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "artifact_projection.lock"

    @staticmethod
    def _metrics_timeline_cache_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "cache" / "metrics_timeline.v1.json"

    @staticmethod
    def _metrics_timeline_cache_lock_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "cache" / "metrics_timeline.lock"

    @staticmethod
    def _baseline_compare_cache_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "cache" / "baseline_compare.v1.json"

    @staticmethod
    def _baseline_compare_cache_lock_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "cache" / "baseline_compare.lock"
    ...
    def _projection_directory(quest_root: Path) -> Path:
        return quest_root / ".ka" / "projections"
    ...
    def _codex_history_events_state(self, quest_root: Path) -> tuple[tuple[str, tuple[int, int, int] | None], ...]:
        return self._glob_states(quest_root / ".ka" / "codex_history", "*/events.jsonl")
    ...
    def _details_projection_state(self, quest_root: Path) -> tuple[Any, ...]:
        ...
        core_paths = [
            ...
            quest_root / ".ka" / "runtime_state.json",
            quest_root / ".ka" / "research_state.json",
            quest_root / ".ka" / "interaction_state.json",
            quest_root / ".ka" / "bindings.json",
            quest_root / ".ka" / "bash_exec" / "summary.json",
    ...
    def _queue_projection_build(self, quest_root: Path, kind: str, *, source_signature: str) -> None:
        ...
        worker = threading.Thread(
            target=_worker,
            name=f"ka-projection-{quest_root.name}-{kind}",
        )
        with self._quest_projection_builds_lock:
    ...
    def _recent_codex_runs(self, quest_root: Path, *, limit: int = 5) -> list[dict[str, Any]]:
        history_root = quest_root / ".ka" / "codex_history"
    ...
    def _analysis_inventory_payload(self, quest_root: Path, workspace_root: Path) -> dict[str, Any] | None:
        ...
        campaigns_root = quest_root / ".ka" / "analysis_campaigns"
        ...
        write_text(
            ...
            "\n".join(
                [
                    ...
                    "Existing runtime traces under `.ka/` were preserved.",
                    "",
                    "## Goal",
                    "",
                    restored_goal,
        ...
        write_text(
            ...
            "\n".join(
                [
                    "# Plan",
                    "",
                    "- [ ] Inspect preserved runtime traces under `.ka/`",
    ...
    def _codex_meta_state(self, quest_root: Path) -> tuple[tuple[str, tuple[int, int, int] | None], ...]:
        return self._glob_states(quest_root / ".ka" / "codex_history", "*/meta.json")
    ...
    def _snapshot_state(self, quest_root: Path) -> tuple[Any, ...]:
        core_paths = [
            ...
            quest_root / ".ka" / "runtime_state.json",
            quest_root / ".ka" / "research_state.json",
            quest_root / ".ka" / "user_message_queue.json",
            quest_root / ".ka" / "interaction_state.json",
            quest_root / ".ka" / "bindings.json",
            quest_root / ".ka" / "conversations" / "main.jsonl",
            quest_root / ".ka" / "bash_exec" / "summary.json",
        ]
        return (
            self._path_states(core_paths),
    ...
    def _snapshot(self, quest_id: str) -> dict:
        ...
        history = self._read_cached_jsonl(quest_root / ".ka" / "conversations" / "main.jsonl")
        artifacts = []
        recent_runs = []
        ...
        codex_history_root = quest_root / ".ka" / "codex_history"
        ...
        paths = {
            ...
            "bash_exec_root": str(quest_root / ".ka" / "bash_exec"),
        }
        counts = {
        ...
        payload = {
            ...
            "cloud": {"linked": False, "base_url": "https://codexscientist.cc"},
    ...
    def append_message(
        ...
        *,
        ...
        interaction_state_path = quest_root / ".ka" / "interaction_state.json"
        interaction_state = self._read_interaction_state(quest_root)
        ...
        append_jsonl(quest_root / ".ka" / "conversations" / "main.jsonl", record)
        if role == "user":
            recent_threads = [dict(item) for item in (interaction_state.get("recent_threads") or [])]
        ...
        if role == "user":
            ...
            if status == "waiting_for_user":
                interaction_state = read_json(quest_root / ".ka" / "interaction_state.json", {"open_requests": []})
    ...
    def history(self, quest_id: str, limit: int = 100) -> list[dict]:
        return self._read_cached_jsonl(self._quest_root(quest_id) / ".ka" / "conversations" / "main.jsonl")[-limit:]
    ...
    def events(
        ...
        *,
        ...
        event_path = self._quest_root(quest_id) / ".ka" / "events.jsonl"
    ...
    def _read_interaction_state(self, quest_root: Path) -> dict[str, Any]:
        state = self._read_cached_json(quest_root / ".ka" / "interaction_state.json", {})
    ...
    def _runtime_state_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "runtime_state.json"

    @staticmethod
    def _agent_status_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "agent_status.json"

    @staticmethod
    def _message_queue_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "user_message_queue.json"

    @staticmethod
    def _interaction_journal_path(quest_root: Path) -> Path:
        return quest_root / ".ka" / "interaction_journal.jsonl"
    ...
    def _write_active_user_requirements(
        ...
        *,
        ...
        user_messages = [
            item
            for item in read_jsonl(quest_root / ".ka" / "conversations" / "main.jsonl")
    ...
    def record_artifact_interaction(
        ...
        *,
        ...
        runtime_updates: dict[str, Any] = {
            ...
            "last_tool_activity_name": "ka_artifact_record",
            "tool_calls_since_last_artifact_interact": 0,
            "pending_user_message_count": len((self._read_message_queue(quest_root).get("pending") or [])),
        }
        ...
        *,
        ...
        if delivered_messages:
            lines = [
                ...
                self.localized_copy(
                    quest_root=quest_root,
                    zh="- 立即通过 Hermes 回复或记录一条有实际内容的 follow-up；不要重复发送只有“已收到/处理中”的确认。",
                    en="- Immediately provide one substantive Hermes follow-up; do not send a redundant receipt-only message such as 'received' or 'processing'.",
                ),
                self.localized_copy(
                    quest_root=quest_root,
                    zh="- 如果可以直接回答，就在这次 follow-up 里直接完整回答。",
                    en="- If you can answer directly, give the full answer in that follow-up.",
                ),
                self.localized_copy(
                    quest_root=quest_root,
                    zh="- 如果暂时不能直接回答，就在这次 follow-up 里说明你将先处理该用户请求，给出简短计划、最近回传点与预计输出。",
                    en="- If you cannot answer directly yet, explain in that follow-up that you will handle this user request first, and include a short plan, nearest report-back point, and expected output.",
                ),
                self.localized_copy(
                    quest_root=quest_root,
                    zh="- 完成该用户请求后，再立刻通过 Hermes 汇报完整结果，并按需使用 ka_artifact_record 记录进展。",
                    en="- After completing that user request, immediately report the full result through Hermes and use ka_artifact_record when a durable progress artifact is needed.",
                ),
                self.localized_copy(
                    quest_root=quest_root,
    ...
    def _snapshot_folder_kind(tree: dict[str, dict | None], relative: str) -> str | None:
        ...
        if not normalized or normalized.startswith(".ka/"):
            return None
    ...
    def _skip_explorer_relative(relative: str) -> bool:
        if relative.startswith(".git/") or relative == ".git":
            return True
        if relative.startswith(".ka/worktrees/"):
            return True
        parts = PurePosixPath(relative).parts
    ...
    def _skip_explorer_profile_relative(relative: str, profile: str | None) -> bool:
        ...
        if top in {".codex", ".claude", ".ka", "tmp", "userfiles", "artifacts"}:
            return True
    ...
    def _classify_relative_scope(relative: str) -> tuple[str, bool]:
        ...
        if relative.startswith(".ka/codex_history/"):
            return "runner_history", False
        if relative.startswith(".ka/"):
