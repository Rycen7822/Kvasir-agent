...
def tinytex_root_candidates(home: Path | None = None) -> list[Path]:
    ...
    for env_name in ("KVASIR_AGENT_TINYTEX_ROOT", "KA_TINYTEX_ROOT", "TINYTEX_DIR"):
...
def inspect_latex_runtime(home: Path | None = None) -> dict[str, Any]:
    ...
    if not available:
        ...
        guidance.append("Install TinyTeX using its official instructions and add its binaries to PATH.")
        guidance.append(
        ...
        guidance.append("Install TinyTeX or add `bibtex` to your system LaTeX distribution.")
...
def _download_installer(url: str) -> tuple[bool, str]:
    request = Request(url, headers={"User-Agent": "Kvasir-agent TinyTeX bootstrap"})
    try:
...
def install_tinytex(home: Path | None = None) -> dict[str, Any]:
    ...
    if sys.platform.startswith("win"):
        ...
        if not which("perl"):
            return {
                ...
                "guidance": [
                    "Install Perl first, then retry TinyTeX installation.",
    ...
    if result.returncode != 0:
        return {
            ...
            "guidance": [
                "Retry TinyTeX installation after checking network connectivity.",
                f"Official docs: {_TINYTEX_DOC_URL}",
            ],
    ...
    if not refreshed.get("tinytex", {}).get("installed"):
        return {
            "ok": False,
            "changed": False,
            "summary": "TinyTeX installer finished, but Kvasir-agent could not find the managed pdflatex runtime afterward.",
