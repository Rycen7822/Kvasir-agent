...
_PROMPT_SYNC_STATE_FILENAME = ".kvasir-agent-prompt-sync.json"
...
class SkillInstaller:
    ...
    def sync_global(self) -> dict:
        ...
        for bundle in self.discover():
            target = codex_root / f"kvasir-agent-{bundle.skill_id}"
    ...
    def _sync_claude_projection(self, bundle, target_root: Path) -> Path:
        target = target_root / f"kvasir-agent-{bundle.skill_id}.md"
    ...
    def _prune_bundle_targets(root: Path, expected_names: set[str]) -> None:
        for target in sorted(root.glob("kvasir-agent-*"), reverse=True):
