...
def looks_like_kvasiragent_root(path: Path) -> bool:
...
def migrate_kvasiragent_root(source: Path, target: Path) -> dict[str, Any]:
    ...
    if not looks_like_kvasiragent_root(source):
        raise ValueError(f"Source path does not look like a Kvasir-agent home or install root: {source}")
    if source == target:
        raise ValueError("Source path and target path must be different.")
    if _is_relative_to(target, source):
        raise ValueError("Target path cannot be placed inside the current Kvasir-agent root.")
    if _is_relative_to(source, target):
        raise ValueError("Target path cannot be a parent of the current Kvasir-agent root.")
    if target.exists():
    ...
    try:
        ...
        return {
            ...
            "summary": "Kvasir-agent root copied and verified successfully.",
        }
    except Exception:
        shutil.rmtree(staging, ignore_errors=True)
        raise
