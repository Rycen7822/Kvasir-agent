...
def codex_runtime_overrides() -> dict[str, str]:
    binary = _as_text(os.environ.get("KVASIR_AGENT_CODEX_BINARY") or os.environ.get("KA_CODEX_BINARY"))
    approval_policy = _as_text(os.environ.get("KVASIR_AGENT_CODEX_APPROVAL_POLICY"))
    sandbox_mode = _as_text(os.environ.get("KVASIR_AGENT_CODEX_SANDBOX_MODE"))
    profile = _as_text(os.environ.get("KVASIR_AGENT_CODEX_PROFILE"))
    model = _as_text(os.environ.get("KVASIR_AGENT_CODEX_MODEL"))

    yolo_enabled = _as_optional_bool_env("KVASIR_AGENT_CODEX_YOLO")
