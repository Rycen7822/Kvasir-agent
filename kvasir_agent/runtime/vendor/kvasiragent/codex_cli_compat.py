...
_COMPAT_BEGIN_MARKER = "# BEGIN KVASIR_AGENT PROFILE COMPAT"
_COMPAT_END_MARKER = "# END KVASIR_AGENT PROFILE COMPAT"
    ...
    *,
    ...
    return (
        ...
        (
            ...
            "Kvasir-agent downgraded reasoning effort to `high` automatically."
        ),
    )
...
def adapt_profile_only_provider_config(
    ...
    *,
    ...
    return (
        adapted,
        (
            f"Kvasir-agent overrode conflicting top-level {field_text} with values from profile "
            f"`{normalized_profile}` for Codex compatibility."
            if conflicted_fields
            else f"Kvasir-agent promoted `{normalized_profile}` profile {field_text} to the top level for Codex compatibility."
        ),
    )
