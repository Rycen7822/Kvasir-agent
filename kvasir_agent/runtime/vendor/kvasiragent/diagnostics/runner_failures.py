    ...
    *,
    ...
    if "missing environment variable" in lower:
        return FailureDiagnosis(
            ...
            guidance=(
                "Set the required key in `~/Kvasir-agent/config/runners.yaml` under `runners.codex.env`.",
    ...
    if any(marker in lower for marker in _MODEL_UNAVAILABLE_MARKERS):
        return FailureDiagnosis(
            ...
            guidance=(
                ...
                "If you need a fixed model, verify that the same model works in plain `codex exec` before retrying Kvasir-agent.",
            ),
            retriable=False,
