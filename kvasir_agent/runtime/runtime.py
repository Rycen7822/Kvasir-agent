"""Kvasir-agent runtime resolver for the Codex adapter."""
...
def _clear_conflicting_kvasiragent_modules(vendor_root: Path) -> None:
    existing = sys.modules.get("kvasiragent")
    if existing is None:
        return
    ...
    for name in list(sys.modules):
        if name == "kvasiragent" or name.startswith("kvasiragent."):
...
def ensure_runtime_import_environment(config: NativeConfig | None = None) -> None:
    ...
    _clear_conflicting_kvasiragent_modules(vendor_root)
    vendor = str(vendor_root)
    sys.path = [p for p in sys.path if p != vendor]
    sys.path.insert(0, vendor)
    # Child monitor processes launched by BashExecService import kvasiragent by module name.
    ...
    os.environ["KVASIR_AGENT_REPO_ROOT"] = repo
    os.environ["KVASIR_AGENT_HOME"] = str(cfg.runtime_home.expanduser())
    os.environ["KA_HOME"] = str(cfg.runtime_home.expanduser())
...
def _import_services():
    from kvasiragent.artifact import ArtifactService
...
def doctor() -> dict[str, Any]:
    ...
    try:
        ensure_runtime_import_environment(cfg)
        import kvasiragent  # type: ignore
        check("vendored_runtime_import", True, "Vendored Kvasir-agent runtime imports successfully.", package_file=str(Path(kvasiragent.__file__).resolve()))
    ...
    check("resource_skills", expected_skills.issubset(present_skills), "Kvasir-agent stage/companion skills are present.", missing=sorted(expected_skills - present_skills), count=len(present_skills))
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="kvasir-agent-doctor-") as tmp:
            probe_home = Path(tmp) / "Kvasir-agent"
            probe_cfg = replace(
                cfg,
    ...
    forbidden = ["src/ui", "src/tui", "vendor/kvasiragent/tui.py", "vendor/kvasiragent/" + "connector" + "_runtime.py", "vendor/kvasiragent/connector", "resources/prompts/connectors", "vendor/kvasiragent/" + ("dae" + "mon")]
    ...
    check("no_external_upstream_cli_required", True, "Codex-native adapter does not require the upstream CLI.")
