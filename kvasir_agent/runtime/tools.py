...
"""Codex-native Kvasir-agent tool handlers.

Handlers call vendored Kvasir-agent services directly. They do not use MCP or require the upstream CLI.
"""
...
def _project_root_arg(args: dict[str, Any]) -> Path:
    ...
    os.environ["KVASIR_AGENT_PROJECT_ROOT"] = str(root)
    return root
...
def _root_bound_memory_service(state_root: Path):
    ...
    memory_module = importlib.import_module("kvasiragent.memory")
...
def ensure_root_bound_vendor_shim(state_root: Path, manifest: dict[str, Any] | None = None) -> None:
    ...
    (state_root / ".ka").mkdir(parents=True, exist_ok=True)
    for rel_dir in (".ka/conversations", ".ka/runs", ".ka/worktrees", ".ka/bash_exec"):
...
def _build_startup_contract(args: dict[str, Any]) -> dict[str, Any] | tuple[None, str]:
    ...
    mode_rationale = str(args.get("mode_rationale") or "").strip() or (
        "default_copilot: workspace_mode was omitted, so the Codex-managed Kvasir-agent quest starts in request-scoped copilot mode without a default paper goal."
...
@_guard
def ka_doctor(args: dict[str, Any]) -> dict[str, Any]:
    return native_doctor()


@_guard
def ka_list_quests(args: dict[str, Any]) -> dict[str, Any]:
    ctx = _current_research(args, create=False)
    if ctx.get("ok"):
        quest = {
...
@_guard
def ka_get_quest_state(args: dict[str, Any]) -> dict[str, Any]:
    ctx = _current_research(args, create=False)
    if not ctx.get("ok"):
...
@_guard
def ka_set_active_quest(args: dict[str, Any]) -> dict[str, Any]:
    ctx = _current_research(args, create=False)
...
@_guard
def ka_new_quest(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_update_quest_mode(args: dict[str, Any]) -> dict[str, Any]:
...
def _add_user_message_payload(args: dict[str, Any]) -> dict[str, Any]:
    ...
    _append_jsonl(root / ".ka" / "conversations" / "main.jsonl", record)
    _append_jsonl(
        root / ".ka" / "events.jsonl",
        {
    ...
    queue_path = root / ".ka" / "user_message_queue.json"
...
@_guard
def ka_add_user_message(args: dict[str, Any]) -> dict[str, Any]:
    return _add_user_message_payload(args)


@_guard
def ka_record_user_requirement(args: dict[str, Any]) -> dict[str, Any]:
    args = dict(args)
...
@_guard
def ka_events(args: dict[str, Any]) -> dict[str, Any]:
    return _read_events_payload(args)


@_guard
def ka_read_quest_documents(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
    if not quest_id:
        return {"ok": False, "error": "quest_id is required"}
...
@_guard
def ka_memory_search(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_memory_read(args: dict[str, Any]) -> dict[str, Any]:
    ctx = _current_research(args, create=False)
    if error := _root_bound_error(ctx):
        return error
    memory_service = _root_bound_memory_service(Path(ctx["state_root"]))
...
@_guard
def ka_memory_list_recent(args: dict[str, Any]) -> dict[str, Any]:
    ctx = _current_research(args, create=False)
    if error := _root_bound_error(ctx):
...
@_guard
def ka_memory_write(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_artifact_record(args: dict[str, Any]) -> dict[str, Any]:
    ctx = _current_research(args, create=True)
    if error := _root_bound_error(ctx):
        return error
    root = Path(ctx["quest_root"])
...
@_guard
def ka_confirm_baseline(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_waive_baseline(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_attach_baseline(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_create_local_baseline(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "quest_id", "baseline_id"):
        return {"ok": False, "error": err}
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
    ...
    if source_path:
        ...
        if not content:
            ...
            summary = str(args.get("summary") or "Local baseline contract created by Hermes-native Kvasir-agent.").strip()
            content = f"# {title}\n\n{summary}\n"
    ...
    return {
        ...
        "next_tool": "ka_confirm_baseline",
        "guidance": "Pass confirm_args to ka_confirm_baseline when this local baseline is ready to open the baseline gate.",
    }


@_guard
def ka_submit_idea(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_list_research_branches(args: dict[str, Any]) -> dict[str, Any]:
...
def _root_bound_confirm_baseline(args: dict[str, Any], *, root: Path, quest_id: str) -> dict[str, Any]:
    ...
    _write_json_file(root / "config" / "baselines" / "entries" / f"{baseline_id}.json", record)
    _append_jsonl(root / "config" / "baselines" / "index.jsonl", record)
    _append_jsonl(root / ".ka" / "events.jsonl", {"type": "baseline.confirmed", **record})
    try:
        from kvasir_agent.services.manifest import ManifestService
...
def _analysis_campaign_path(root: Path, campaign_id: str) -> Path:
    return root / ".ka" / "analysis_campaigns" / f"{_safe_slug(campaign_id, 'active')}.json"
...
def _root_bound_get_analysis_campaign(args: dict[str, Any], *, root: Path, quest_id: str) -> dict[str, Any]:
    ...
    path = _analysis_campaign_path(root, campaign_id)
    if not path.exists():
        matches = sorted((root / ".ka" / "analysis_campaigns").glob("*.json"))
        if len(matches) == 1:
            path = matches[0]
...
@_guard
def ka_resolve_runtime_refs(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
    branch = ""
...
@_guard
def ka_get_paper_contract_health(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
    return {
...
@_guard
def ka_get_global_status(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
    return {
...
@_guard
def ka_get_method_scoreboard(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
...
@_guard
def ka_get_optimization_frontier(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
    from kvasir_agent.services.method_improvement import MethodImprovementService
...
@_guard
def ka_get_conversation_context(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
    limit = _limit(args, default=12, maximum=200)
    messages: list[dict[str, Any]] = []
    path = root / ".ka" / "conversations" / "main.jsonl"
    if path.exists():
...
@_guard
def ka_record_main_experiment(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_create_analysis_campaign(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_get_analysis_campaign(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "quest_id"):
        return {"ok": False, "error": err}
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
...
@_guard
def ka_record_analysis_slice(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_submit_paper_outline(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "quest_id"):
        return {"ok": False, "error": err}
    services = get_services()
    quest_id, root, error = _artifact_root_from_args(args, services)
...
@_guard
def ka_list_paper_outlines(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
...
@_guard
def ka_submit_paper_bundle(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "quest_id"):
        return {"ok": False, "error": err}
    services = get_services()
    quest_id, root, error = _artifact_root_from_args(args, services)
...
@_guard
def ka_arxiv(args: dict[str, Any]) -> dict[str, Any]:
    quest_id, root, error = _artifact_root_from_args(args)
    if error:
        return error
...
@_guard
def ka_environment_register(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_environment_validate(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_environment_show(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "env_id"):
        return missing
    from kvasir_agent.services.environment import EnvironmentService
...
@_guard
def ka_trajectory_record(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id"):
        return missing
    ...
    return {"ok": False, "error": f"Unknown ka_trajectory_record operation: {operation}", "error_type": "invalid_operation", "recoverable": True}


@_guard
def ka_trajectory_search(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id"):
        return missing
    from kvasir_agent.services.trajectory import TrajectoryStore
...
@_guard
def ka_trajectory_show(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_evolutionary_plan_round(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "env_id"):
        return missing
...
_MCP_EXECUTOR_GATE_GRANTED: ContextVar[bool] = ContextVar("ka_mcp_executor_gate_granted", default=False)
...
@_guard
def ka_variant_create(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "env_id", "trajectory_id", "idea_id"):
        return missing
    gate = _executor_gate(args, tool_name="ka_variant_create")
...
@_guard
def ka_variant_apply_patch(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "variant_id", "patch_path"):
        return missing
    gate = _executor_gate(args, tool_name="ka_variant_apply_patch")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.variant import VariantService
...
@_guard
def ka_variant_check(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "variant_id"):
        return missing
    gate = _executor_gate(args, tool_name="ka_variant_check")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.variant import VariantService
...
@_guard
def ka_variant_pack(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "variant_id"):
        return missing
    gate = _executor_gate(args, tool_name="ka_variant_pack")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.variant import VariantService
...
@_guard
def ka_scheduler_submit(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "env_id", "trajectory_id", "variant_id", "package_path", "command"):
        return missing
    gate = _runtime_executor_gate(args, tool_name="ka_scheduler_submit")
    if gate.get("ok") is not True:
        return gate
...
@_guard
def ka_scheduler_status(args: dict[str, Any]) -> dict[str, Any]:
    gate = _runtime_executor_gate(args, tool_name="ka_scheduler_status")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.scheduler import SchedulerService
...
@_guard
def ka_worker_claim(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "worker_id"):
        return missing
    gate = _runtime_executor_gate(args, tool_name="ka_worker_claim")
    if gate.get("ok") is not True:
        return gate
...
@_guard
def ka_worker_heartbeat(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "run_id"):
        return missing
    gate = _runtime_executor_gate(args, tool_name="ka_worker_heartbeat")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.worker import WorkerService
...
@_guard
def ka_worker_upload_artifact(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "job_id", "artifact_path"):
        return missing
    gate = _runtime_executor_gate(args, tool_name="ka_worker_upload_artifact")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.worker import WorkerService
...
@_guard
def ka_worker_collect(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "job_id"):
        return missing
    gate = _runtime_executor_gate(args, tool_name="ka_worker_collect")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.worker import WorkerService
...
@_guard
def ka_evolutionary_round_submit(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "env_id", "round_id", "submissions"):
        return missing
    gate = _runtime_executor_gate(args, tool_name="ka_evolutionary_round_submit")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.evolutionary import EvolutionarySearchService
...
@_guard
def ka_implementer_patch_check(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "variant_id", "patch_path"):
        return missing
    gate = _executor_gate(args, tool_name="ka_implementer_patch_check")
    if gate.get("ok") is not True:
        return gate
    from kvasir_agent.services.variant import VariantService
...
@_guard
def ka_implementer_repair_patch(args: dict[str, Any]) -> dict[str, Any]:
    if missing := _required_payload(args, "quest_id", "variant_id"):
        return missing
    gate = _executor_gate(args, tool_name="ka_implementer_repair_patch")
    if gate.get("ok") is not True:
        return gate
...
@_guard
def ka_feedback_ingest(args: dict[str, Any]) -> dict[str, Any]:
...
@_guard
def ka_bash_exec(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    ...
    if operation != "run":
        return {"ok": False, "error": f"Unknown ka_bash_exec operation: {operation}"}
    if err := _require(args, "command"):
        return {"ok": False, "error": err}
    from kvasiragent.execution_context import ExecutionContext
    ...
    if allow_project_root:
        env_payload.setdefault("KVASIR_AGENT_PROJECT_ROOT", str(project_root))
...
@_guard
def ka_workflow_smoke_report(args: dict[str, Any]) -> dict[str, Any]:
    """Return a lightweight Kvasir-agent full-workflow checklist.
    ...
    return {
        ...
        "ready": ready,
        "summary": "Hermes-only Kvasir-agent smoke checklist covers dataset inspection, baseline, experiment, analysis, paper_bundle, and final report summary.",
    }
...
Broadly collect candidates here before deep reading. Run `ka_paper_reliability_verify` for each candidate once the pool is large enough, using the bundled `paper_reliability_verifier`, then mark status.
...
def _download_url_to_bytes(url: str) -> bytes:
    req = Request(url, headers={"User-Agent": "Kvasir-agent-Hermes/0.2 paper-fetch"})
...
@_guard
def ka_strict_research_prepare(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
    if not quest_id:
    ...
    workflow = [
        "广泛查阅相关论文并用 ka_strict_research_upsert_candidate 维护 candidate_references.md；此阶段不急着深读。",
        "候选数量足够后，对候选分小 batch 调用 ka_paper_reliability_verify；每篇返回 paper/tier/quality_flags/warnings 和 reliability_card_path。",
        "每完成一个小 batch，立刻用 ka_strict_research_upsert_candidate 更新 candidate_references.md 中对应论文的 status、Reliability card、Retain/reject reason，再进入下一 batch。",
        ...
        "保留列表足够后用 ka_paper_fetch 将 PDF 下载到 reference/pdfs/，记录 canonical_url、sha256、page_count 和 ledger。",
        "调用 ka_strict_research_init_bibliography 创建 bibliography 三文件。",
        "逐篇精读；每读完一篇调用 ka_record_literature_reading_note 记录 read_status、sections_read、claim_routes，并立刻更新 bibliography 三文件，再进入下一篇。",
    ...
    return {
        ...
        "workflow": workflow,
        "verifier_skill": "kvasiragent:paper-reliability-verifier",
        "verifier_resource_path": str(_plugin_root() / "resources" / "skills" / "paper-reliability-verifier"),
    }


@_guard
def ka_strict_research_record_candidate(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "title"):
        return {"ok": False, "error": err}
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
...
@_guard
def ka_strict_research_upsert_candidate(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
    if not quest_id:
        return {"ok": False, "error": "quest_id is required when no root-bound research manifest exists"}
...
@_guard
def ka_paper_fetch(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
    if not quest_id:
        return {"ok": False, "error": "quest_id is required when no root-bound research manifest exists"}
...
@_guard
def ka_record_literature_reading_note(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
    if not quest_id:
        return {"ok": False, "error": "quest_id is required when no root-bound research manifest exists"}
...
@_guard
def ka_strict_research_init_bibliography(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
    if not quest_id:
        return {"ok": False, "error": "quest_id is required when no root-bound research manifest exists"}
...
def _load_bundled_verifier(verifier_root: Path) -> Any:
    ...
    module_name = f"_kvasiragent_bundled_paper_verifier_{abs(hash(script))}"
...
@_guard
def ka_paper_reliability_verify(args: dict[str, Any]) -> dict[str, Any]:
    services = get_services()
    quest_id = _root_bound_manifest_quest_id(args, services)
    if not quest_id:
        return {"ok": False, "error": "quest_id is required when no root-bound research manifest exists"}
...
@_guard
def ka_pause_quest(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "quest_id"):
        return {"ok": False, "error": err}
    services = get_services()
...
@_guard
def ka_resume_quest(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "quest_id"):
        return {"ok": False, "error": err}
    services = get_services()
...
@_guard
def ka_stop_quest(args: dict[str, Any]) -> dict[str, Any]:
    if err := _require(args, "quest_id"):
        return {"ok": False, "error": err}
    services = get_services()
