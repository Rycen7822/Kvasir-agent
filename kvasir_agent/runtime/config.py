"""Configuration for the Kvasir-agent native plugin.

This module intentionally does not resolve or require the upstream CLI.
...
PROJECT_HOME_DIR_NAME = "Kvasir-agent"
PROJECT_CONFIG_FILE_NAME = "codex-native.yaml"


def _launch_workdir() -> Path:
    configured = str(os.environ.get("KVASIR_AGENT_PROJECT_ROOT") or os.environ.get("KVASIR_AGENT_PROJECT_ROOT") or "").strip()
...
def _legacy_config_root_override() -> Path | None:
    configured = str(os.environ.get("KVASIR_AGENT_HERMES_ROOT") or "").strip()
...
def _default_config_path() -> Path:
    configured = str(os.environ.get("KVASIR_AGENT_HERMES_CONFIG") or "").strip()
    if configured:
...
def default_config_text() -> str:
    return """# By default the Codex-native adapter uses the current project directory.
# Launch Codex from a project directory and Kvasir-agent state is stored in:
#   ./Kvasir-agent/
# Uncomment runtime_home only if this project intentionally needs a custom root.
# runtime_home: ./Kvasir-agent
mode:
