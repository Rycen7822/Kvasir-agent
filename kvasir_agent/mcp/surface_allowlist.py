...
_FORBIDDEN_AGENT_FACING_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (_literal("scripts/", "ka", "ctl.py"), re.compile(re.escape(_literal("scripts/", "ka", "ctl.py")), re.IGNORECASE)),
    (_literal("CLI ", "fallback"), re.compile(re.escape(_literal("CLI ", "fallback")), re.IGNORECASE)),
    (_literal("ka", "ctl services"), re.compile(re.escape(_literal("ka", "ctl services")), re.IGNORECASE)),
    (_literal("current ", "ka", "ctl surface"), re.compile(re.escape(_literal("current ", "ka", "ctl surface")), re.IGNORECASE)),
    (_literal("ka", "ctl"), re.compile(r"(?<![A-Za-z0-9_-])" + re.escape(_literal("ka", "ctl")) + r"(?![A-Za-z0-9_-])", re.IGNORECASE)),
)
...
_ALLOWED_CLI_REFERENCE_PATHS = (
    ...
    "kvasir_agent/adapters/legacy_kactl.py",
    "tests/test_json_output_flag.py",
    "tests/test_kactl_compat.py",
