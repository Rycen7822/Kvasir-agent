...
def _service(args: dict[str, Any]) -> tuple[GoalLoopService, KvasirAgentMcpContext]:
    context = KvasirAgentMcpContext.from_env(args)
    return GoalLoopService(context.resolve_project_layout()), context


def _quest_id(context: KvasirAgentMcpContext, args: dict[str, Any]) -> str | None:
...
def goal_state(args: dict[str, Any]) -> dict[str, Any]:
    ...
    if not quest_id:
        return {"ok": False, "error": "quest_id is required for ka_goal_state", "error_type": "missing_argument", "recoverable": True}
...
def goal_next_action(args: dict[str, Any]) -> dict[str, Any]:
    ...
    quest_id = _quest_id(context, args)
    if not quest_id:
        return {"ok": False, "error": "quest_id is required for ka_goal_next_action", "error_type": "missing_argument", "recoverable": True}
