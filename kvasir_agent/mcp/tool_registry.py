"""Curated MCP tool registry for Kvasir-agent."""
...
from kvasir_agent.services.project_state import ProjectLayout, ProjectRootResolver
from kvasir_agent.services.progress_watchdog import ProgressWatchdogService
...
_EXECUTOR_MCP_ENV = "KVASIR_AGENT_ENABLE_EXECUTOR_MCP"
...
def _minimal_schema_from_spec(spec: ToolSpec) -> dict[str, Any]:
    ...
    elif spec.name == "ka_context_pack":
        extra_properties.update({
            "quest_id": _minimal_property_for_key("quest_id"),
            "max_chars": {"type": "integer", "default": 12000},
        })
    elif spec.name == "ka_artifact_index":
        extra_properties.update({
            "quest_id": _minimal_property_for_key("quest_id"),
...
_HANDLERS: dict[str, Callable[[dict[str, Any]], dict[str, Any]]] = {
    ...
    "ka_context_pack": _context_pack,
    "ka_resume_brief": _resume_brief,
    "ka_checkpoint": _checkpoint,
    "ka_goal_watchdog": _goal_watchdog,
    "ka_pack_delta": _pack_delta,
    "ka_manifest_init": research_tools.manifest_init,
    "ka_manifest_record_baseline": research_tools.manifest_record_baseline,
    "ka_manifest_validate": _manifest_validate,
    "ka_queue_submit": research_tools.queue_submit,
    "ka_queue_start_attempt": research_tools.queue_start_attempt,
    "ka_queue_status": _queue_status,
    "ka_queue_reconcile": _queue_reconcile,
    "ka_runner_start": research_tools.runner_start,
    "ka_runner_status": _runner_status,
    "ka_log_digest": _log_digest,
    "ka_artifact_index": _artifact_index,
    "ka_trial_propose": research_tools.trial_propose,
    "ka_trial_plan": research_tools.trial_plan,
    "ka_trial_ready": research_tools.trial_ready,
    "ka_trial_evaluate": research_tools.trial_evaluate,
    "ka_trial_decide": research_tools.trial_decide,
    "ka_trial_show": _trial_show,
    "ka_wiki_query_pack": _wiki_query_pack,
    "ka_review_status": _review_status,
...
_HANDLERS["ka_submit_idea"] = research_tools.submit_idea
_HANDLERS["ka_record_main_experiment"] = research_tools.record_main_experiment
_HANDLERS["ka_record_negative_result"] = research_tools.record_negative_result
_HANDLERS["ka_update_method_scoreboard"] = research_tools.update_method_scoreboard
_HANDLERS["ka_select_next_idea"] = research_tools.select_next_idea
_HANDLERS["ka_claim_gate"] = research_tools.claim_gate
...
_SPECS: list[ToolSpec] = [
    ...
    _spec("ka_resume_brief", "Return stable low-token recovery anchors for a project.", group="checkpoint"),
    ...
    _spec("ka_memory_read", "Read one quest-local memory card by id or path.", group="memory", required=("quest_id",)),
    _spec("ka_memory_list_recent", "List recent quest-local memory cards.", group="memory", required=("quest_id",)),
    _spec("ka_memory_write", "Write a quest-local memory card.", group="memory", read_only=False, idempotent=False, required=("quest_id", "title")),
    _spec("ka_artifact_record", "Record a generic artifact.", group="artifact", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_create_local_baseline", "Create a local baseline stub.", group="baseline", read_only=False, idempotent=False, required=("quest_id", "baseline_id")),
    _spec("ka_confirm_baseline", "Confirm a baseline gate.", group="baseline", read_only=False, idempotent=False, required=("quest_id", "baseline_path")),
    _spec("ka_submit_idea", "Submit or revise an idea candidate with a novelty contract.", group="idea", read_only=False, idempotent=False, required=("quest_id", "title", "novelty_contract")),
    _spec("ka_get_method_scoreboard", "Read or refresh the method scoreboard.", group="idea", read_only=False, required=("quest_id",)),
    _spec("ka_get_optimization_frontier", "Read optimization frontier summary.", group="idea", required=("quest_id",)),
    ...
    _spec("ka_record_analysis_slice", "Record an analysis slice result.", group="analysis", read_only=False, idempotent=False, required=("quest_id", "campaign_id", "slice_id")),
    _spec("ka_bash_exec", "Run/list/read/wait/stop quest-local bash sessions.", group="experiment", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_submit_paper_outline", "Submit/select/revise a paper outline.", group="paper", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_submit_paper_bundle", "Submit a paper bundle manifest.", group="paper", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_refresh_summary", "Refresh SUMMARY.md from recent state.", group="paper", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_paper_fetch", "Fetch official paper PDF into the quest library.", group="paper", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_arxiv", "Read or list the quest-local arXiv library.", group="literature", required=("quest_id",)),
    _spec("ka_environment_register", "Register an execution-grounded environment manifest.", group="execution_planning", read_only=False, idempotent=False, required=("quest_id", "manifest")),
    _spec("ka_environment_validate", "Validate an execution-grounded environment manifest.", group="execution_planning", required=("quest_id", "env_id")),
    _spec("ka_environment_show", "Read an execution-grounded environment manifest.", group="execution_planning", required=("quest_id", "env_id")),
    _spec("ka_feedback_ingest", "Ingest local execution feedback metrics/logs into a trajectory.", group="execution_feedback", read_only=False, idempotent=False, required=("quest_id", "env_id", "trajectory_id", "run_id", "source_kind")),
    _spec("ka_trajectory_record", "Create or update an execution-grounded trajectory record.", group="trajectory", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_trajectory_search", "Search execution-grounded trajectories.", group="trajectory", required=("quest_id",)),
    _spec("ka_trajectory_show", "Read one execution-grounded trajectory.", group="trajectory", required=("quest_id", "trajectory_id")),
    _spec("ka_evolutionary_plan_round", "Create a deterministic plan-only evolutionary round; never submits jobs or creates variants.", group="execution_planning", read_only=False, idempotent=True, required=("quest_id", "env_id")),
    _spec("ka_variant_create", "Create an isolated execution variant worktree behind executor approval gates.", group="executor", read_only=False, idempotent=False, required=("quest_id", "env_id", "trajectory_id", "idea_id")),
    _spec("ka_variant_apply_patch", "Apply a patch inside an isolated variant workspace behind executor approval gates.", group="executor", read_only=False, idempotent=False, required=("quest_id", "variant_id", "patch_path")),
    _spec("ka_variant_check", "Run smoke checks for a variant behind executor approval gates.", group="executor", read_only=False, idempotent=False, required=("quest_id", "variant_id")),
    _spec("ka_variant_pack", "Create a deterministic variant package behind executor approval gates.", group="executor", read_only=False, idempotent=False, required=("quest_id", "variant_id")),
    _spec("ka_scheduler_submit", "Submit one executor-local scheduler job for a packed variant.", group="executor", read_only=False, idempotent=False, required=("quest_id", "env_id", "trajectory_id", "variant_id", "package_path", "command")),
    _spec("ka_scheduler_status", "Read executor-local scheduler queue status.", group="executor", required=()),
    _spec("ka_worker_claim", "Claim and start one executor-local scheduler job.", group="executor", read_only=False, idempotent=False, required=("worker_id",)),
    _spec("ka_worker_heartbeat", "Record a heartbeat for an executor-local worker run.", group="executor", read_only=False, idempotent=True, required=("run_id",)),
    _spec("ka_worker_collect", "Collect one executor-local worker job and ingest metrics/log feedback.", group="executor", read_only=False, idempotent=False, required=("job_id",)),
    _spec("ka_worker_upload_artifact", "Copy one local worker artifact into the quest execution-grounded artifact area.", group="executor", read_only=False, idempotent=False, required=("job_id", "artifact_path")),
    _spec("ka_evolutionary_round_submit", "Submit scheduler jobs for an existing approved EvolutionaryRoundPlan; never creates variants.", group="executor", read_only=False, idempotent=False, required=("quest_id", "env_id", "round_id", "submissions")),
    _spec("ka_implementer_patch_check", "Dry-run an implementer patch against a variant behind executor approval gates.", group="executor", read_only=False, idempotent=False, required=("quest_id", "variant_id", "patch_path")),
    _spec("ka_implementer_repair_patch", "Return gated patch-repair guidance for an implementer failure.", group="executor", read_only=False, idempotent=False, required=("quest_id", "variant_id")),
    _spec("ka_strict_research_prepare", "Initialize strict literature research mode for a quest.", group="literature", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_strict_research_record_candidate", "Append a strict-research candidate paper row.", group="literature", read_only=False, idempotent=False, required=("quest_id", "title")),
    _spec("ka_strict_research_upsert_candidate", "Upsert a strict-research candidate paper row.", group="literature", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_record_literature_reading_note", "Record a strict-research reading note.", group="literature", read_only=False, idempotent=False, required=("quest_id", "title")),
    _spec("ka_strict_research_init_bibliography", "Initialize strict-research bibliography working files.", group="literature", read_only=False, idempotent=False, required=("quest_id",)),
    _spec("ka_paper_reliability_verify", "Verify and store one paper reliability evidence card.", group="literature", read_only=False, idempotent=False, required=("quest_id", "title")),
    _spec("ka_manifest_init", "Initialize a project-local research manifest.", group="manifest", read_only=False, idempotent=False, required=("name", "goal")),
    _spec("ka_manifest_record_baseline", "Record manifest baseline readiness.", group="manifest", read_only=False, idempotent=False, required=("baseline_id",)),
    _spec("ka_manifest_validate", "Validate the project-local research manifest.", group="manifest", read_only=False),
    _spec("ka_queue_submit", "Submit a local runner queue job.", group="queue", read_only=False, idempotent=False, required=("job_id", "command")),
    _spec("ka_queue_start_attempt", "Start a queued job attempt.", group="queue", read_only=False, idempotent=False, required=("job_id",)),
    _spec("ka_queue_status", "Read queue status.", group="queue"),
    _spec("ka_queue_reconcile", "Reconcile expired queue leases.", group="queue", read_only=False),
    _spec("ka_runner_start", "Start or dry-run a project-local runner record.", group="runner", read_only=False, idempotent=False, required=("command",)),
    _spec("ka_runner_status", "Read one or all runner records.", group="runner"),
    _spec("ka_log_digest", "Return a bounded redacted digest for one runner log.", group="runner", required=("run_id",)),
    _spec("ka_artifact_index", "Return artifact references, hashes, sizes, and types without file content.", group="artifact"),
    _spec("ka_trial_propose", "Propose a trial.", group="trial", read_only=False, idempotent=False, required=("quest_id", "idea_id", "hypothesis")),
    _spec("ka_trial_plan", "Plan a proposed trial.", group="trial", read_only=False, idempotent=False, required=("trial_id",)),
    _spec("ka_trial_ready", "Move a planned trial through readiness gates.", group="trial", read_only=False, idempotent=False, required=("trial_id",)),
    _spec("ka_trial_evaluate", "Evaluate a ready trial against metric contracts.", group="trial", read_only=False, idempotent=False, required=("trial_id",)),
    _spec("ka_trial_decide", "Keep or revert an evaluated trial.", group="trial", read_only=False, idempotent=False, required=("trial_id",)),
    _spec("ka_trial_show", "Read one trial record by id.", group="trial", required=("trial_id",)),
    _spec("ka_wiki_query_pack", "Build a bounded research wiki query pack.", group="wiki"),
    _spec("ka_review_status", "Read review artifact status.", group="review"),
    _spec("ka_cost_status", "Read latest cost and approval gate status.", group="cost"),
    _spec("ka_soak_accelerated", "Run accelerated fake-clock long-run validation.", group="soak", read_only=False, idempotent=False),
    _spec("ka_soak_crash_resume", "Record restart and reconcile expired leases.", group="soak", read_only=False, idempotent=False),
]
...
def _default_error_action(error_type: str) -> str:
    ...
    if error_type == "unknown_tool":
        return "Call tools/list and choose one of the curated ka_* tools."
...
def _analysis_campaign_retry_template(args: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "name": "ka_create_analysis_campaign",
...
def _analysis_campaign_preflight(args: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if missing:
        return _error_payload(
            ...
            suggested_next_action="Retry ka_create_analysis_campaign with the missing writing-facing fields; omit all writing-facing fields only for evidence-only campaigns.",
        )
    return None
...
def _normalized_failure_payload(payload: dict[str, Any], *, tool_name: str | None, args: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    current["error"] = error
    if tool_name == "ka_bash_exec" and error == "workdir_outside_quest":
        ...
        current["suggested_next_action"] = "Retry ka_bash_exec with workdir under the quest root, or omit workdir and keep cwd_policy=quest."
    if tool_name == "ka_confirm_baseline" and "baseline_path" in error and ("state_root" in error or "quest_root" in error):
        current.setdefault("retry_template", {
            "name": "ka_confirm_baseline",
            "required_arguments": ["quest_id", "baseline_path"],
            "path_constraint": "baseline_path must be under state_root; use ka_create_local_baseline to create a canonical root-bound baseline first.",
        })
        current["suggested_next_action"] = "Use ka_create_local_baseline, then retry ka_confirm_baseline with the returned confirm_args."
    if tool_name in {"ka_record_main_experiment", "ka_create_analysis_campaign", "ka_record_analysis_slice", "ka_claim_gate"} and (
        "artifact.confirm_baseline" in error or "artifact.waive_baseline" in error
    ):
        current["error"] = error.replace("artifact.confirm_baseline(...)", "ka_confirm_baseline").replace("artifact.waive_baseline(...)", "ka_waive_baseline")
        current.setdefault("retry_template", {
            "name": "ka_confirm_baseline_or_ka_waive_baseline",
            "options": ["ka_confirm_baseline", "ka_waive_baseline"],
            "required_before_retry": [tool_name],
        })
        current["suggested_next_action"] = "Open the baseline gate with ka_confirm_baseline, or explicitly record a waiver with ka_waive_baseline, then retry this MCP tool."
    if error_type == "not_found" and tool_name == "ka_get_analysis_campaign":
...
_MEMORY_TOOLS = frozenset({"ka_memory_search", "ka_memory_read", "ka_memory_list_recent", "ka_memory_write"})
...
def _artifact_record_preflight(args: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return _error_payload(
        ...
        retry_template={"name": "ka_artifact_record", "kind": "report", "payload": retry_payload},
        suggested_next_action="Retry ka_artifact_record with a canonical kind. For dataset_inspection or metric_report use kind=report and set payload.report_type to the semantic subtype.",
    )
...
def _paper_reliability_preflight(args: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not title:
        return _error_payload(
            "missing_argument",
            "Provide at least one of title, doi, arxiv_url, or url for ka_paper_reliability_verify.",
            True,
            "ka_paper_reliability_verify",
    ...
    if not dry_run and network_enabled:
        ...
        if has_external_url and not has_resolvable_identifier:
            return _error_payload(
                ...
                "ka_paper_reliability_verify",
                retry_template={
                    "name": "ka_paper_reliability_verify",
    ...
    return {
        ...
        "suggested_next_action": "Retry ka_paper_reliability_verify without dry_run/network=false after official paper metadata is available and external IO is acceptable.",
    }
...
def _bash_exec_preflight(args: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if operation == "run":
        ...
        if missing:
            spec = _SPECS_BY_NAME["ka_bash_exec"]
            payload = _missing_argument_payload("ka_bash_exec", spec, missing, args)
        ...
        if command_class not in _FORMAL_BASH_COMMAND_CLASSES:
            return {
                "ok": False,
                "error": f"Unsupported ka_bash_exec command_class for formal-run provenance: {command_class}",
...
def mcp_tool_not_registered_payload(name: str) -> dict[str, Any]:
    ...
    return _finalize_tool_payload(
        _error_payload(
            ...
            suggested_next_action="Call tools/list and choose one of the registered public Kvasir-agent MCP tools; hidden admin/autonomous tools are not callable over default MCP.",
...
def call_tool(name: str, args: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if handler is None:
        return _finalize_tool_payload(
            _error_payload(
                ...
                True,
                name,
                suggested_next_action="Run ka_doctor and implement or enable this MCP tool family; keep hidden admin/debug CLI outside the default agent research flow.",
            ),
            tool_name=name,
            args=args,
        )
    call_args = dict(args or {})
    if name == "ka_paper_reliability_verify":
    ...
    if name == "ka_create_analysis_campaign":
