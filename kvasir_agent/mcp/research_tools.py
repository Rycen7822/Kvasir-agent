...
Wrappers in this module call Kvasir-agent Python services or native in-process
...
MCP_ONLY_SCHEMAS: tuple[dict[str, Any], ...] = (
    native_schemas.KA_RECORD_NEGATIVE_RESULT,
    native_schemas.KA_UPDATE_METHOD_SCOREBOARD,
    native_schemas.KA_SELECT_NEXT_IDEA,
    native_schemas.KA_CLAIM_GATE,
)
...
@contextmanager
def mcp_environment(args: dict[str, Any]) -> Iterator[KvasirAgentMcpContext]:
    ...
    os.environ["KA_HOME"] = str(research_root)
    os.environ["KA_QUEST_ROOT"] = str(research_root)
    if identity.get("ok"):
        os.environ["KA_QUEST_ID"] = str(identity["quest_id"])
    else:
        os.environ.pop("KA_QUEST_ID", None)
    if context.run_id:
        os.environ["KA_RUN_ID"] = context.run_id
    if context.active_stage:
        os.environ["KA_ACTIVE_STAGE"] = context.active_stage
    if context.conversation_id:
        os.environ["KA_CONVERSATION_ID"] = context.conversation_id
    if context.worktree_root:
        os.environ["KA_WORKTREE_ROOT"] = str(context.worktree_root)
    try:
        yield context
    finally:
...
def _finalize(payload: dict[str, Any], tool_name: str) -> dict[str, Any]:
    ...
    payload["transport"] = "kvasir-agent-mcp"
    payload["mcp"] = True
...
def _layout(args: dict[str, Any]):
    return KvasirAgentMcpContext.from_env(args).resolve_project_layout()
...
def _novelty_contract_retry_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    current["suggested_next_action"] = f"Retry ka_submit_idea with a complete novelty_contract; {missing}."
    return current
...
def record_main_experiment(args: dict[str, Any]) -> dict[str, Any]:
    payload = native_tool_call("ka_record_main_experiment", args)
...
def manifest_init(args: dict[str, Any]) -> dict[str, Any]:
    with mcp_environment(args) as context:
        ...
        return service.init(
            name=str(args.get("name") or args.get("project_name") or "Kvasir-agent"),
...
def _trial_id_contract_error(error_type: str, trial_id: str = "") -> dict[str, Any]:
    if error_type == "missing_argument":
        return {
            ...
            "retry_template": {"name": "ka_trial_show", "required_arguments": ["trial_id"], "missing_arguments": ["trial_id"]},
            "suggested_next_action": "Provide trial_id for ka_trial_show, or call ka_trial_propose to create a new trial.",
        }
    if error_type == "invalid_trial_id":
        return {
            ...
            "recoverable": True,
            "required_context_keys": ["trial_id"],
            "retry_template": {"name": "ka_trial_show", "required_arguments": ["trial_id"], "missing_arguments": []},
            "suggested_next_action": "Retry ka_trial_show with a trial_id formatted like T0001, or call ka_trial_propose to create a new trial.",
        }
    return {
        "ok": False,
        ...
        "required_context_keys": ["trial_id"],
        "retry_template": {
            "name": "ka_trial_propose",
            "required_arguments": ["quest_id", "idea_id", "hypothesis"],
            "missing_arguments": "required_arguments minus keys already present in args",
        },
        "suggested_next_action": "Use ka_trial_show to verify an existing trial_id, or call ka_trial_propose to create a new trial.",
    }
