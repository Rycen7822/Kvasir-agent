...
class VariantService:
    ...
    def _exclude_from_package(self, rel_path: str) -> bool:
        ...
        blocked_dirs = {".git", "__pycache__", ".pytest_cache", ".mypy_cache", ".ruff_cache", "Kvasir-agent", ".cache", "cache", "caches", "secrets", ".secrets"}
    ...
    def _create_snapshot_worktree(self, *, baseline_root: Path, workspace: Path) -> dict[str, Any]:
        shutil.copytree(
            baseline_root,
            workspace,
            ignore=shutil.ignore_patterns(".git", "Kvasir-agent", "__pycache__", ".pytest_cache", ".mypy_cache", ".ruff_cache"),
        )
        ...
        commit = _run_git(
            ["-c", "user.name=Kvasir-agent", "-c", "user.email=kvasiragent@example.invalid", "commit", "--allow-empty", "-m", "baseline snapshot"],
            cwd=workspace,
        )
