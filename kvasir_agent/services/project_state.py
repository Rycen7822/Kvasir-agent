...
class ProjectRootResolver:
    ...
    def resolve(args: dict[str, Any] | None = None) -> Path:
        ...
        env_root = os.environ.get("KVASIR_AGENT_PROJECT_ROOT")
        if env_root:
            return ProjectRootResolver._explicit_path(env_root, label="KVASIR_AGENT_PROJECT_ROOT")
        cwd = Path.cwd().resolve()
        manifest_parent = ProjectRootResolver._nearest_parent_with(cwd, Path("Kvasir-agent") / "research.yaml")
...
class ResearchLayout:
    ...
    def state_root(self) -> Path:
        return self.project_root / "Kvasir-agent"
...
@dataclass(frozen=True)
class ProjectLayout:
    """Canonical project-local paths for Kvasir-agent state."""
