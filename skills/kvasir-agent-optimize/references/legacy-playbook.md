...
- Codex-native execution boundary: use Codex-native tools for routine file inspection, Git mechanics, dependency checks, smoke checks, quick validations, tests, and process management. Use `ka_bash_exec` for optimization runs whose logs must become Kvasir-agent quest evidence, especially formal evaluations, long runs, or frontier-shifting candidate evidence.
...
## Required native kactl-driven workflow
...
- `ka_artifact_record payload={'kind': 'decision', 'action': 'branch'|'continue'|'stop', ...})`
...
- implementation-level candidate attempt inside one line:
  - `ka_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})`
- real measured main result:
  - `ka_artifact_record_main_experiment(...)`
- route change after the result:
  - `ka_artifact_record payload={'kind': 'decision', 'action': 'iterate'|'branch'|'continue'|'stop', ...})`
...
- Use Codex-native execution for routine smoke checks and quick validations; use `ka_bash_exec` for long or formal optimization runs that need durable quest-local provenance.
...
- Use `ka_bash_exec mode='detach', ...)` for long runs and monitor with `list/read/await`.
...
Treat a candidate brief as the Kvasir-agent form of a method brief.
...
- `ka_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})` for within-line attempts
- `ka_artifact_record payload={'kind': 'decision', 'action': 'iterate'|'branch'|'continue'|'stop', ...})` for route changes
- `ka_artifact_record_main_experiment(...)` for real measured line results
...
- Use Codex-native execution for routine smoke checks; use `ka_bash_exec` for full runs or formal optimization evidence that must be auditable in the quest.
