...
- Codex-native execution boundary: use Codex-native tools for routine setup inspection, Git mechanics, dependency checks, and non-evidence verification. Use `cs_bash_exec` for baseline setup, reproduction, monitoring, or verification commands whose logs must be durable CodexScientist evidence.
- Prefer `cs_bash_exec` for setup, reproduction, monitoring, and verification commands so the baseline line stays durable and auditable.
...
- in the Codex-native plugin, prefer `cs_create_local_baseline` for local stubs and then pass its `confirm_args` to `cs_confirm_baseline`
...
- baseline registry index: `~/CodexScientist/config/baselines/index.jsonl`
- canonical baseline entry: `~/CodexScientist/config/baselines/entries/<baseline_id>.yaml`
...
- once the smoke test passes, launch the real baseline reproduction with `cs_bash_exec mode='detach', ...)`
...
- if you need to recover ids or inspect session state, use `cs_bash_exec mode='history')` or `cs_bash_exec mode='list')`
- `cs_bash_exec mode='read', id=...)` returns the full saved log when it is `2000 lines or fewer`; for longer logs, inspect omitted middle windows with `start` and `tail`
- during monitoring, prefer `cs_bash_exec mode='read', id=..., tail_limit=..., order='desc')`, and after the first read prefer incremental checks with `after_seq=last_seen_seq`
- use `silent_seconds`, `progress_age_seconds`, `signal_age_seconds`, and `watchdog_overdue` as the default staleness clues
- if a run is clearly invalid, wedged, or superseded, stop it with `cs_bash_exec mode='kill', id=..., wait=true, timeout_seconds=...)`, document why, and relaunch cleanly
- do not let more than the `30-minute visibility bound` pass without a real inspection and a `next expected update time`
- when the baseline code is under your control, prefer a throttled `tqdm` progress reporter and periodic `__CS_PROGRESS__` markers when feasible
...
- by default, begin every baseline pass with `cs_memory_search(scope='quest', limit=5)`
- then run at least one baseline-relevant `cs_memory_search(...)` before new baseline analysis, repair, or rerun work
...
When calling `cs_memory_write(...)`, pass `tags` as an array like `["stage:baseline", "baseline:<baseline_id>", "type:repro-lesson"]`, not as one comma-joined string.

Stage-end requirement:

- if baseline work produced a durable reproduction lesson, verification caveat, environment incident, or route rationale, write at least one `cs_memory_write(...)` before leaving the stage
