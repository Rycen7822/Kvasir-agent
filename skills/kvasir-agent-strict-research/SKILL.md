---
name: kvasir-agent-strict-research
description: "Strict literature research mode for Kvasir-agent projects: broad candidate scouting first, reliability verification, conservative filtering, project-local PDF download, then iterative bibliography reading notes before answering/writing."
version: 0.1.0
author: Codex Agent
---
...
# Kvasir-agent Strict Research Mode
...
The bundled plugin skill `kvasiragent:paper-reliability-verifier` is available for detailed rules. Use `ka_paper_reliability_verify` to run its verifier in the quest reference workspace. For large surveys, verify candidates in small batches and immediately write the summarized result back to `candidate_references.md` with `ka_strict_research_upsert_candidate`.
