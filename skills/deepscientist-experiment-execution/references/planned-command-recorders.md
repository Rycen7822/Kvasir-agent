...
Use this reference when a DeepScientist experiment command document advances into runner sections whose current CLI implementation is still a `planned_not_executed` recorder.
...
- E06 `score-candidates`
  ...
  - GAAS, GAAS+Coda, CFC, ETD-score, RR-score, RawRepeat, and Middle-cycle remained experiment-internal comparator scores, not formal DeepScientist baselines.
- E07 `run-phase0`
...
4. Record the blocked formal command as a DeepScientist main experiment or milestone/report, but phrase the outcome as blocked/unavailable, not completed.
...
7. Record a DeepScientist milestone after validation, stating explicitly that no real model computation or downstream artifacts were produced when the status is `planned_not_executed`.
