...
Use this note when a DeepScientist Phase 2 `main_100m`/full-budget runner has moved from a single bounded corpus sample to repeated resume chunks and needs to prove that later invocations advance through the streaming corpus instead of rereading the same local split/sample rows.
...
- `recover_target_records.jsonl` does not exist unless formal `Y_recover` was truly written.
...
- saying the experiment plan is complete when `recover_target_records.jsonl` is absent or `writes_y_recover=false`
