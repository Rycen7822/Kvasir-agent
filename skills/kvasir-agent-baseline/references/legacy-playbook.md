...
- Codex-native execution boundary: use Codex-native tools for routine setup inspection, Git mechanics, dependency checks, and non-evidence verification. Use `ka_bash_exec` for baseline setup, reproduction, monitoring, or verification commands whose logs must be durable Kvasir-agent evidence.
- Prefer `ka_bash_exec` for setup, reproduction, monitoring, and verification commands so the baseline line stays durable and auditable.
...
- **Use Codex-native execution for routine shell/CLI/Python/node/git/npm/uv/environment checks. Use `ka_bash_exec` for baseline setup, reproduction, monitoring, or verification that must be durable quest evidence.**
...
- **If a generic git smoke test is needed outside the quest repo, use `ka_bash_exec ...)` in an isolated scratch repository.**
...
- in the Codex-native plugin, prefer `ka_create_local_baseline` for local stubs and then pass its `confirm_args` to `ka_confirm_baseline`
...
- baseline registry index: `~/Kvasir-agent/config/baselines/index.jsonl`
- canonical baseline entry: `~/Kvasir-agent/config/baselines/entries/<baseline_id>.yaml`
...
- once the smoke test passes, launch the real baseline reproduction with `ka_bash_exec mode='detach', ...)`
...
- if you need to recover ids or inspect session state, use `ka_bash_exec mode='history')` or `ka_bash_exec mode='list')`
- `ka_bash_exec mode='read', id=...)` returns the full saved log when it is `2000 lines or fewer`; for longer logs, inspect omitted middle windows with `start` and `tail`
- during monitoring, prefer `ka_bash_exec mode='read', id=..., tail_limit=..., order='desc')`, and after the first read prefer incremental checks with `after_seq=last_seen_seq`
- use `silent_seconds`, `progress_age_seconds`, `signal_age_seconds`, and `watchdog_overdue` as the default staleness clues
- if a run is clearly invalid, wedged, or superseded, stop it with `ka_bash_exec mode='kill', id=..., wait=true, timeout_seconds=...)`, document why, and relaunch cleanly
- do not let more than the `30-minute visibility bound` pass without a real inspection and a `next expected update time`
- when the baseline code is under your control, prefer a throttled `tqdm` progress reporter and periodic `__KA_PROGRESS__` markers when feasible
