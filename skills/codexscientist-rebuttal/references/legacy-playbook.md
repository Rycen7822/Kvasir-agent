...
- When the rebuttal plan, the main supplementary-evidence package, or the final response bundle becomes durable, send one richer `cs_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` update that says what reviewer concerns are now addressed, what still remains open, and what happens next.
- Codex-native execution boundary: use Codex-native tools for routine manuscript inspection, scripted checks, Git mechanics, and local edits. Use `cs_bash_exec` for reviewer-linked experiment launches or rebuttal evidence builds whose logs must become CodexScientist provenance.
...
Stay inside the normal CodexScientist surface: `memory`, `artifact`, `cs_bash_exec`, plus ordinary stage/companion skills.
...
6. after each slice finishes, immediately `cs_artifact_record_analysis_slice(...)`
...
If ids or refs are unclear, recover them first with `cs_artifact_record evidence path resolution(...)`, `artifact.get_analysis_campaign(...)`, or `artifact.list_paper_outlines(...)`.
...
- run at least one `cs_memory_search(...)` for:
  - paper title
  - main method name
...
- if the rebuttal pass produced a durable lesson, claim downgrade, or reviewer-driven route change, write at least one `cs_memory_write(...)`
