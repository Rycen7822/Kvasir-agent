...
---
name: kvasir-agent-review
description: Run a skeptical evidence-grounded Kvasir-agent review pass for drafts, paper-like reports, claim scope, and follow-up routing.
version: 1.0.1
...
- Codex-native execution boundary: use Codex-native file/shell/Git/test/build tools for routine document builds, scripted checks, Git inspection, and file inspection. Use public MCP `ka_bash_exec` only when the command itself must become quest evidence; ordinary Codex file tools are preferred for direct file IO.
...
- `paper-like-idea-revision.md` for revising a substantial idea/report from external researcher feedback while preserving first-author voice, compact section-level edits, Kvasir-agent durability, canonical artifact kinds, source fetching/caveats, final mechanical checks, and reviewer-memo pitfalls such as target leakage, baseline category errors, direct-trial controls, and infeasible experiment scale
...
- `paper-like-idea-revision.md` also covers repeated Kvasir-agent split-idea revision hygiene, including `idea/` directory layout: active docs in root, audits in `idea/audits/`, backups in `idea/backup/`, and legacy/source carryover in `idea/archive/`.
