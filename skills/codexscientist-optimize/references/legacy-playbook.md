...
This skill is the lightweight optimization control layer for CodexScientist.
It does not replace the normal quest runtime. It tells you how to use the existing CodexScientist artifact, memory, cs_bash_exec, Git, and worktree mechanisms as an optimization system.
...
## Required native csctl-driven workflow
...
1. `cs_artifact_record / analysis artifact lookup(...)`
2. `cs_memory_search(scope='quest', limit=5)`
3. `cs_memory_search(...)`
...
- `cs_artifact_record payload={'kind': 'decision', 'action': 'branch'|'continue'|'stop', ...})`
...
- implementation-level candidate attempt inside one line:
  - `cs_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})`
- real measured main result:
  - `cs_artifact_record_main_experiment(...)`
- route change after the result:
  - `cs_artifact_record payload={'kind': 'decision', 'action': 'iterate'|'branch'|'continue'|'stop', ...})`
...
- Use `cs_bash_exec mode='detach', ...)` for long runs and monitor with `list/read/await`.
...
- Use `cs_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})` for implementation-level candidate attempts inside one durable line.
- Before deciding the next route, call `cs_artifact_record / analysis artifact lookup(...)` when available and use it as the primary optimization-state summary.
...
- run `cs_memory_search(scope='quest', limit=5)`
- run at least one `cs_memory_search(...)`
- read `cs_artifact_record / analysis artifact lookup(...)`
- update `OPTIMIZE_CHECKLIST.md`
...
Treat a candidate brief as the CodexScientist form of a method brief.
...
Before broad new search, run at least one `cs_memory_search(...)` using:
...
- `cs_artifact_record payload={'kind': 'report', 'report_type': 'optimization_candidate', ...})` for within-line attempts
- `cs_artifact_record payload={'kind': 'decision', 'action': 'iterate'|'branch'|'continue'|'stop', ...})` for route changes
- `cs_artifact_record_main_experiment(...)` for real measured line results
...
- Use Codex-native execution for routine smoke checks; use `cs_bash_exec` for full runs or formal optimization evidence that must be auditable in the quest.
...
- write at least one `cs_memory_write(...)` when the pass produced a reusable success pattern, repeated failure pattern, fusion lesson, or explicit non-retry rule
- update `OPTIMIZE_CHECKLIST.md`
...
- [ ] Read `cs_artifact_record / analysis artifact lookup(...)` or equivalent durable frontier summary
