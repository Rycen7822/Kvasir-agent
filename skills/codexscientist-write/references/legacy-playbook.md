...
This skill intentionally absorbs the strongest old CodexScientist writing discipline, including:
...
- no active analysis campaign still has pending required slices; in the Codex-native plugin call `cs_get_analysis_campaign` before outline selection or paper bundle submission when campaign state is uncertain
...
Read `templates/CODEXSCIENTIST_NOTES.md` for the local selection guide and `templates/README.md` for the upstream template notes.
...
- save compile logs, preferably through `cs_bash_exec` session ids or exported `cs_bash_exec` logs
...
- begin every writing pass with `cs_memory_search(scope='quest', limit=5)`
- then run at least one write-relevant `cs_memory_search(...)` before drafting, major revision, or claim restructuring
...
When calling `cs_memory_write(...)`, pass `tags` as an array like `["stage:write", "type:writing-playbook", "type:evidence-ledger"]`, not as one comma-joined string.
...
- if writing produced a durable citation lesson, review lesson, claim-boundary rule, or packaging constraint, write at least one `cs_memory_write(...)` before leaving the stage
