...
Use this reference when a CodexScientist formal experiment has moved past Phase 2 selector setup and the next boundary is a real retrofit runner, but the full paper-scale training/evaluation chain may still be unavailable.
...
4. Run the command through `cs_bash_exec`, validate artifacts from disk, then record milestone/decision with `cs_artifact_record`.
