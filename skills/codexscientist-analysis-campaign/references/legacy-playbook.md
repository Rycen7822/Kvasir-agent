...
---
name: codexscientist-analysis-campaign
...
This is the shared CodexScientist protocol for supplementary experiments after a durable result.
...
It preserves the core old CodexScientist analysis-experimenter discipline:
...
8. Before returning to `write`, outline selection, or paper bundle submission, use the Codex-native `cs_get_analysis_campaign` tool if available and verify there is no active campaign with pending required slices.
...
- `cs_bash_exec` session ids and managed shell logs for campaign runs
...
After the charter and launch decision are durably recorded, send one threaded `cs_artifact_record payload={'kind': 'milestone'}, ...)` update naming:
...
After each slice finishes, call `cs_artifact_record_analysis_slice(...)` immediately so the result is mirrored back to the parent branch and the next slice can be activated.
If a slice fails or becomes infeasible, still call `cs_artifact_record_analysis_slice(...)` with an honest non-success status plus the real blocker and next recommendation; do not leave the campaign state ambiguous.
...
Each `cs_artifact_record_analysis_slice(...)` call should also include an `evaluation_summary` with exactly these six fields:
...
- once the smoke test passes, launch the real slice with `cs_bash_exec mode='detach', ...)` and normally leave `timeout_seconds` unset for that long run
...
- if you need a middle section that was omitted from that default preview, use `cs_bash_exec mode='read', id=..., start=..., tail=...)`
- monitor them with `cs_bash_exec mode='list')` and `cs_bash_exec mode='read', id=..., tail_limit=..., order='desc')`
- after the first read, prefer `cs_bash_exec mode='read', id=..., after_seq=last_seen_seq, tail_limit=..., order='asc')` for incremental monitoring
- if ids become unclear, recover them through `cs_bash_exec mode='history')`
- launch long slices with a structured `comment` such as `{stage, goal, action, expected_signal, next_check}`
- use `silent_seconds`, `progress_age_seconds`, `signal_age_seconds`, and `watchdog_overdue` from `cs_bash_exec mode='list'|'read', ...)` as the default stall checks
...
- after the first meaningful signal and then at real checkpoints (e.g., completion, blocker, recovery, or a materially changed evidence frontier), send `cs_artifact_record payload={'kind': 'progress'}, ...)` so the user sees the newest real state
- after each completed sleep / await monitoring cycle for an active slice, inspect state first; only send another `cs_artifact_record payload={'kind': 'progress'}, ...)` update if the user-visible state materially changed
- include the estimated next reply time or next check time in those monitoring updates
- stop them with `cs_bash_exec mode='kill', id=..., wait=true, timeout_seconds=...)` if the slice is invalid, wedged, or superseded; add `force=true` when immediate termination is required
- when you control the slice code, prefer a throttled `tqdm` progress reporter and, when feasible, pair it with concise `__CS_PROGRESS__` lines carrying phase and ETA
...
- working surface:
  - `.cs/worktrees/<slice_id>/` when isolated worktrees are used
- experiment surface:
...
Before a long slice starts, emit a `progress` artifact or `cs_artifact_record payload={'kind': 'progress'}, ...)` update so the quest shows that the slice is active.
...
Send a threaded `cs_artifact_record payload={'kind': 'milestone'}, ...)` update at that point with the new boundary or risk.
...
When the aggregated campaign report is complete, send a richer threaded `cs_artifact_record payload={'kind': 'milestone'}, ...)` update.
...
- begin every analysis campaign pass with `cs_memory_search(scope='quest', limit=5)`
- then run at least one analysis-relevant `cs_memory_search(...)` before launching or resuming slices
...
- if the campaign produced a durable cross-slice lesson, failure pattern, or comparability caveat, write at least one `cs_memory_write(...)` before leaving the stage
