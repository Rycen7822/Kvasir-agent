...
- Keep ordinary subtask completions concise. When the idea stage actually finishes a meaningful deliverable such as a selected idea package, a rejected-ideas summary, or a route-shaping ideation checkpoint, upgrade to a richer `cs_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` report.
...
This stage should preserve the strongest old CodexScientist direction-selection logic:
...
- Before opening a broad new search, check quest and global memory with `cs_memory_search(...)` and reuse existing paper notes, idea notes, and knowledge cards.
...
- quest and global memory cards returned by `cs_memory_search(...)` and `cs_memory_search(...)`
...
- `cs_memory_search(...)` and recent memory reads
...
When the stage reaches a route-shaping outcome, notify the user through `cs_artifact_record ...)` deliberately:
...
When calling `cs_memory_write(...)`, pass `tags` as an array like `["stage:idea", "type:selection-rationale", "topic:<mechanism>"]`, not as one comma-joined string.

Recommended read timing:

- before any new paper search:
  - run `cs_memory_search(...)` over the baseline, task, dataset, mechanism, and current idea labels
...
- if ideation produced a durable survey conclusion, selected-idea rationale, rejected-idea lesson, or novelty caveat, write at least one `cs_memory_write(...)` before leaving the stage
