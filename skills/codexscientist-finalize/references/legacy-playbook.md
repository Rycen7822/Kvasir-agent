...
- Codex-native execution boundary: use Codex-native tools for routine Git inspection, packaging checks, document builds, and file inspection. Use `cs_bash_exec` only for final closure validations whose logs must be retained as CodexScientist evidence.
...
- begin every finalize pass with `cs_memory_search(scope='quest', limit=5)`
- then run at least one finalize-relevant `cs_memory_search(...)` before closure writing
...
- if finalize produced a durable cross-quest lesson worth reusing later, write at least one `cs_memory_write(...)` before leaving the stage
