...
- Persist durable research facts through `cs_*` calls when they affect root-bound research state, memory, artifacts, baselines, experiments, reviews, analysis, method improvement, or evidence.
...
2. If no research state exists, let the first durable write lazily create it; normally record stable user constraints with `cs_record_user_requirement`.
