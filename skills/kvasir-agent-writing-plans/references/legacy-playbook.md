...
---
name: kvasir-agent-writing-plans
description: Write Kvasir-agent-aware implementation, experiment-roadmap, code-only, and formal-command plans with TDD and durable quest outputs.
version: 1.1.0
author: Kvasir-agent Codex adapter (adapted from obra/superpowers)
license: MIT
metadata:
  hermes:
...
### Kvasir-agent experiment roadmap variant

When the plan is for a Kvasir-agent quest's experiment execution rather than a normal software feature (see `references/kvasir-agent-experiment-roadmaps.md` for the session-tested checklist):

1. Load the relevant Kvasir-agent stage skill and follow the Codex-native operation boundary: use Codex-native tools for routine shell/Git/Python checks, and use `ka_bash_exec` only for formal evidence commands that need quest-local provenance. Use `ka_artifact_record` / `ka_memory_write` for durable outputs.
...
When the user asks to implement experiment code from an existing roadmap but explicitly says not to run experiments, use `references/kvasir-agent-code-only-implementation-passes.md`.
...
3. Order the document by the experiment phases (for Kvasir-agent, commonly E00-E12), starting with global shell variables and directory creation.
