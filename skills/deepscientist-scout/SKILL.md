...
- When scouting actually resolves the framing ambiguity, locks the evaluation contract, or makes the next anchor obvious, send one richer `ds_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` update that says what is now clear, why it matters, and which stage should come next.
...
- Before broad external search, check quest/global memory first with `ds_memory_search(...)` and `ds_memory_search(...)`.
...
- begin every scout pass with `ds_memory_search(scope='quest', limit=5)`
- then run at least one scout-relevant `ds_memory_search(...)` before broad new search
...
When calling `ds_memory_write(...)`, pass `tags` as an array like `["stage:scout", "type:related-work", "topic:<task-or-dataset>"]`, not as one comma-joined string.

Recommended read timing:

- before any new web search:
  - run `ds_memory_search(...)` over task, benchmark, metric, split, and likely baselines
- at scout start:
...
- if scouting produced a durable framing conclusion, paper note, shortlist lesson, or metric-contract caveat, write at least one `ds_memory_write(...)` before leaving the stage
...
Use `ds_artifact_record ...)` for a structured decision request only when a real ambiguity remains and local evidence cannot safely resolve it.
