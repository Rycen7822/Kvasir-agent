...
- If `startup_contract.decision_policy = autonomous`, do not emit ordinary `ka_artifact_record payload={'kind': 'decision_request'}, ...)` calls; decide the route yourself, record the reason, and continue.
...
- Quest completion is a special terminal decision: first ask for explicit completion approval with `ka_artifact_record payload={'kind': 'decision_request'}, reply_mode='blocking', reply_schema={'decision_type': 'quest_completion_approval'}, ...)`, and only after an explicit approval reply should you call `a Codex-controlled finalize recommendation record(...)`.
...
- **If decision-making needs formal shell/CLI/Python/node/git/npm/uv/environment evidence for the quest record, gather it through `ka_bash_exec`; otherwise Codex-native checks are acceptable and the final decision should be recorded through `ka_artifact_record`.**
...
- **Use `decision` to judge the route, not as an excuse to bypass the `ka_bash_exec ...)` / `quest-local git state via backend service or Codex-controlled git action(...)` tool contract.**
...
- return to an older durable branch without creating a new node -> `a Codex-controlled branch activation action(...)`
...
- start the next optimization round from a measured result -> `ka_artifact_record payload={'kind': 'decision', 'action': 'iterate', ...})`
- launch analysis campaign -> `artifact.create_analysis_campaign(...)`
- finish one analysis slice -> `ka_artifact_record_analysis_slice(...)`
...
When a decision materially changes the route, follow it with the appropriate user-visible `ka_artifact_record ...)` update:

- use threaded `ka_artifact_record payload={'kind': 'milestone'}, reply_mode='threaded', ...)` when the decision is already durably resolved and the quest can continue automatically
...
Use `ka_artifact_record payload={'kind': 'decision', ...})` for the final decision.

If user input is needed, also use `ka_artifact_record payload={'kind': 'decision_request'}, ...)`.
