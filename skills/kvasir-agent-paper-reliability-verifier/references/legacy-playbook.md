...
Bundled local ranking snapshots are read from this skill's `paper_ranking/` directory by default.
It must contain `conference_ranking.csv` and `journal_ranking.csv`.
Set `PAPER_RANKING_DIR` to use an updated external snapshot.
