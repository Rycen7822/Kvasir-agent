---
name: deepscientist-quest-handoffs
...
metadata:
  hermes:
    category: research
    tags: [handoff, deepscientist, research, agents, synchronization, verification]
...
> When a handoff becomes durable, record it with `ds_artifact_record` as a milestone/report and include the absolute quest path in the payload.
...
## DeepScientist workflow

1. Read quest state with DeepScientist tools when available.
...
- Do not overwrite target-local project configuration while syncing, such as `DeepScientist/config/hermes-native.yaml`, unless the user explicitly asks.
