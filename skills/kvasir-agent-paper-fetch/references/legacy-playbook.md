...
---
name: kvasir-agent-paper-fetch
description: Use when a Kvasir-agent quest needs quest-local paper retrieval, arXiv/OpenReview PDF reading, or official resource verification without clipping into llm-wiki.
skill_role: support
version: 1.0.0
author: Codex Agent
license: MIT
metadata:
  hermes:
    tags: [kvasiragent, research, arxiv, openreview, pdf, literature]
...
This support skill is for **Kvasir-agent research work**, not note-taking archival.
Use it to retrieve, read, and verify papers for a quest, then persist the findings into Kvasir-agent memory, artifacts, baseline records, literature reports, or paper evidence ledgers.

Keep this skill separate from `clip`:

- `clip` archives sources into `llm-wiki` / Obsidian-style raw notes.
- `paper-fetch` gets paper evidence for the current Kvasir-agent quest.
...
- a Kvasir-agent `scout`, `baseline`, `idea`, `write`, `finalize`, or `rebuttal` stage needs a specific paper read closely;
...
## Output Contract for Kvasir-agent
...
1. Try Kvasir-agent/Codex paper reading primitives if available, e.g. `artifact.arxiv(paper_id=..., full_text=False)` for actual arXiv reading.
...
Do not bring these archive-specific behaviors into Kvasir-agent paper fetching:
