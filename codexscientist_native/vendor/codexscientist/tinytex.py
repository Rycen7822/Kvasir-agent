...
def tinytex_root_candidates(home: Path | None = None) -> list[Path]:
    ...
    for env_name in ("CODEXSCIENTIST_TINYTEX_ROOT", "CS_TINYTEX_ROOT", "TINYTEX_DIR"):
...
def inspect_latex_runtime(home: Path | None = None) -> dict[str, Any]:
    ...
    if not available:
        ...
        guidance.append("Install a lightweight TinyTeX runtime with `cs latex install-runtime`.")
        guidance.append(
        ...
        guidance.append("Install TinyTeX with `cs latex install-runtime` or add `bibtex` to your system LaTeX distribution.")
...
def _download_installer(url: str) -> tuple[bool, str]:
    request = Request(url, headers={"User-Agent": "CodexScientist TinyTeX bootstrap"})
    try:
...
def install_tinytex(home: Path | None = None) -> dict[str, Any]:
    ...
    if sys.platform.startswith("win"):
        ...
        if not which("perl"):
            return {
                ...
                "guidance": [
                    "Install Perl first, then rerun `cs latex install-runtime`.",
    ...
    if result.returncode != 0:
        return {
            ...
            "guidance": [
                "Retry `cs latex install-runtime` after checking network connectivity.",
                f"Official docs: {_TINYTEX_DOC_URL}",
            ],
    ...
    if not refreshed.get("tinytex", {}).get("installed"):
        return {
            "ok": False,
            "changed": False,
            "summary": "TinyTeX installer finished, but CodexScientist could not find the managed pdflatex runtime afterward.",
