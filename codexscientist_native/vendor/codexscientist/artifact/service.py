...
def get_channel_factory(name: str):
    raise RuntimeError("CodexScientist connector channels are disabled in Hermes-headless backend.")
...
class ArtifactService:
    ...
    def _analysis_manifest_path(self, quest_root: Path, campaign_id: str) -> Path:
        return ensure_dir(quest_root / ".cs" / "analysis_campaigns") / f"{campaign_id}.json"
    ...
    def _baseline_workspace_roots(self, quest_root: Path) -> list[Path]:
        ...
        research_state = read_json(quest_root / ".cs" / "research_state.json", {})
        ...
        *,
        ...
        if not fetched.get("ok"):
            ...
            return {
                ...
                "message": "Metadata is temporarily unavailable. Open the arXiv link directly while CodexScientist retries later.",
        ...
        if completion_request is None:
            return {
                ...
                "message": (
                    ...
                    "Ask the user through Hermes and record the decision with a curated cs_* tool; "
    ...
    def _recent_inbound_messages(self, quest_root: Path, *, limit: int) -> list[dict]:
        conversation_path = quest_root / ".cs" / "conversations" / "main.jsonl"
    ...
    def _interaction_state_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "interaction_state.json"
