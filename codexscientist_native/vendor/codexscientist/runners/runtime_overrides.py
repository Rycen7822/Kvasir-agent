...
def codex_runtime_overrides() -> dict[str, str]:
    binary = _as_text(os.environ.get("CODEXSCIENTIST_CODEX_BINARY") or os.environ.get("CS_CODEX_BINARY"))
    approval_policy = _as_text(os.environ.get("CODEXSCIENTIST_CODEX_APPROVAL_POLICY"))
    sandbox_mode = _as_text(os.environ.get("CODEXSCIENTIST_CODEX_SANDBOX_MODE"))
    profile = _as_text(os.environ.get("CODEXSCIENTIST_CODEX_PROFILE"))
    model = _as_text(os.environ.get("CODEXSCIENTIST_CODEX_MODEL"))

    yolo_enabled = _as_optional_bool_env("CODEXSCIENTIST_CODEX_YOLO")
