...
class QuestLatexService:
    ...
    def _folder_build_root(self, project_id: str, folder_relative: str) -> Path:
        quest_root = self._quest_root(project_id)
        return ensure_dir(quest_root / ".cs" / "latex_builds" / _sanitize_folder_key(folder_relative))
            ...
            build.update(
                {
                    ...
                    "error_message": (
                        ...
                        "Install TinyTeX with `cs latex install-runtime` or install a system LaTeX distribution."
                    ),
                    "log_ready": True,
