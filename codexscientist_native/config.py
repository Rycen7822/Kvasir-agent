"""Configuration for the CodexScientist native plugin.

This module intentionally does not resolve or require a global `cs` binary.
...
PROJECT_HOME_DIR_NAME = "CodexScientist"
PROJECT_CONFIG_FILE_NAME = "codex-native.yaml"


def _launch_workdir() -> Path:
    configured = str(os.environ.get("CODEXSCIENTIST_PROJECT_ROOT") or os.environ.get("CODEXSCIENTIST_PROJECT_ROOT") or "").strip()
...
def _legacy_config_root_override() -> Path | None:
    configured = str(os.environ.get("CODEXSCIENTIST_HERMES_ROOT") or "").strip()
...
def _default_config_path() -> Path:
    configured = str(os.environ.get("CODEXSCIENTIST_HERMES_CONFIG") or "").strip()
    if configured:
...
class NativeConfig:
    ...
    use_codexscientist_memory: bool = True
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "memory": {
                "use_codexscientist_memory": self.use_codexscientist_memory,
...
def load_config(config_path: Path | None = None) -> NativeConfig:
    ...
    return NativeConfig(
        ...
        use_codexscientist_memory=_as_bool(memory_data.get("use_codexscientist_memory"), True),
...
def default_config_text() -> str:
    return """# By default the Codex-native adapter follows upstream `cs --here` semantics.
# Launch Codex from a project directory and CodexScientist state is stored in:
#   ./CodexScientist/
# Uncomment runtime_home only if this project intentionally needs a custom root.
# runtime_home: ./CodexScientist
mode:
...
state:
  ...
  # session_map_path: ./CodexScientist/runtime/codex-session-map.json
memory:
  use_codexscientist_memory: true
