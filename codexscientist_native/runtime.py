"""CodexScientist runtime resolver for the Codex adapter."""
...
def _clear_conflicting_codexscientist_modules(vendor_root: Path) -> None:
    existing = sys.modules.get("codexscientist")
    if existing is None:
        return
    ...
    for name in list(sys.modules):
        if name == "codexscientist" or name.startswith("codexscientist."):
...
def ensure_runtime_import_environment(config: NativeConfig | None = None) -> None:
    ...
    _clear_conflicting_codexscientist_modules(vendor_root)
    vendor = str(vendor_root)
    sys.path = [p for p in sys.path if p != vendor]
    sys.path.insert(0, vendor)
    # Child monitor processes launched by BashExecService import codexscientist by module name.
...
def _import_services():
    from codexscientist.artifact import ArtifactService
    from codexscientist.bash_exec import BashExecService
    from codexscientist.config import ConfigManager
    from codexscientist.home import ensure_home_layout
    from codexscientist.memory import MemoryService
    from codexscientist.quest import QuestService
    from codexscientist.registries import BaselineRegistry
    from codexscientist.skills import SkillInstaller
    return {
...
def doctor() -> dict[str, Any]:
    ...
    try:
        ensure_runtime_import_environment(cfg)
        import codexscientist  # type: ignore
        check("vendored_runtime_import", True, "Vendored CodexScientist runtime imports successfully.", package_file=str(Path(codexscientist.__file__).resolve()))
    ...
    check("no_external_cs_required", True, "Codex-native adapter does not require a global npm cs binary.")
