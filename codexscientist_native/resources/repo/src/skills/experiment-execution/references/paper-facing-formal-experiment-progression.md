...
Use this note when the user asks to take over or continue a CodexScientist formal experiment and defines success as completing all paper-facing main figures/tables with formal data.
...
5. Run via `cs_bash_exec` with a stable runstamp and bounded limits.
...
7. Record `cs_record_main_experiment` for formal main experiments and `cs_artifact_record(kind="milestone"|"decision"|"report")` for surrounding decisions/reports.
