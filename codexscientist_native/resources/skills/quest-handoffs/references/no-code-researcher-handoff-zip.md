# No-code researcher handoff zip for research/CodexScientist quests
...
- selected CodexScientist artifact records for provenance.
...
- `.cs/` runtime logs and the full `.git/` repository;
...
## CodexScientist artifact note

For document/package maintenance, prefer recording a `milestone` rather than a generic `report`. In one Quest 001 session, `cs_artifact_record(kind="report")` semantically matched an old report artifact and did not create a clear new record, while `kind="milestone"` produced the intended durable checkpoint.
