---
name: review
description: Run a skeptical evidence-grounded CodexScientist review pass for drafts, paper-like reports, claim scope, and follow-up routing.
version: 1.0.1
...
- `paper-like-idea-revision.md` for revising a substantial idea/report from external researcher feedback while preserving first-author voice, compact section-level edits, CodexScientist durability, canonical artifact kinds, source fetching/caveats, final mechanical checks, and reviewer-memo pitfalls such as target leakage, baseline category errors, direct-trial controls, and infeasible experiment scale
...
- `paper-like-idea-revision.md` also covers repeated CodexScientist split-idea revision hygiene, including `idea/` directory layout: active docs in root, audits in `idea/audits/`, backups in `idea/backup/`, and legacy/source carryover in `idea/archive/`.
...
- `cs_bash_exec` for quest-logged shell; ordinary file tools for direct file IO, `read_file`, `search_files`, and `patch`
...
- if the review produced a durable lesson, claim downgrade, revision rule, or experiment-gap judgment that will matter across sessions, write a concise `cs_memory_write` card or durable local note
