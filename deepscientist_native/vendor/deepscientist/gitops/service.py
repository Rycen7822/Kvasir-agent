...
def _ensure_local_git_identity(repo: Path) -> None:
    ...
    if not name:
        run_command(["git", "config", "--local", "user.name", "DeepScientist Agent"], cwd=repo, check=False)
    if not email:
        run_command(["git", "config", "--local", "user.email", "deepscientist@example.invalid"], cwd=repo, check=False)
