...
Use this note when a DeepScientist quest has progressed from Phase 2 selector/corpus provenance and a planned retrofit recorder to a real but bounded recurrent-core adapter smoke run. It captures how to keep advancing E10 without overclaiming G2.
...
7. Record a milestone and decision after validation. If `ds_record_main_experiment` fails on optional chart rendering, use validated disk artifacts plus `ds_artifact_record` for durable routing rather than rerunning the experiment blindly.
...
- `recover_target_records.jsonl` is absent until formal candidate-level recovery is justified.
