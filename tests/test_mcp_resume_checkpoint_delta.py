...
def test_mcp_checkpoint_resume_and_delta_core_flow_matches_cli(tmp_path: Path):
    _run_kactl(tmp_path, "manifest", "init", "--name", "demo", "--goal", "resume goal")

    cli_checkpoint = _run_kactl(
        tmp_path,
        "summary",
        "checkpoint",
        "--phase",
    ...
    cli_resume = _run_kactl(tmp_path, "summary", "resume-brief", "--max-chars", "4000")
    mcp_resume = call_tool("ka_resume_brief", {"project": str(tmp_path), "max_chars": 4000})
