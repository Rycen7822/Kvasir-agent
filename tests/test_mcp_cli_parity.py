...
def run_kactl(project: Path, *args: str) -> dict:
...
def test_mcp_manifest_validate_matches_cli_core_fields(tmp_path: Path):
    run_kactl(tmp_path, "manifest", "init", "--name", "demo", "--goal", "test goal")

    cli = run_kactl(tmp_path, "manifest", "validate")
    mcp = call_tool("ka_manifest_validate", {"project": str(tmp_path)})

    assert mcp["ok"] == cli["ok"]
    assert mcp["baseline_ready"] == cli["baseline_ready"]
    assert mcp["errors"] == cli["errors"]
    assert mcp["path"] == cli["path"]
    assert mcp["transport"] == "kvasir-agent-mcp"
    assert mcp["mcp"] is True


def test_mcp_queue_status_matches_cli_core_fields(tmp_path: Path):
    run_kactl(tmp_path, "queue", "submit", "--job-id", "job1", "--command", "python train.py")

    cli = run_kactl(tmp_path, "queue", "status")
    mcp = call_tool("ka_queue_status", {"project": str(tmp_path)})
...
def test_mcp_context_pack_matches_cli_core_fields(tmp_path: Path):
    ...
    cli = run_kactl(tmp_path, "summary", "context-pack", "--max-chars", "2000")
    mcp = call_tool("ka_context_pack", {"project": str(tmp_path), "max_chars": 2000})
...
def test_mcp_trial_show_matches_cli_core_fields(tmp_path: Path):
    ...
    cli = run_kactl(tmp_path, "trial", "show", trial_id)
    mcp = call_tool("ka_trial_show", {"project": str(tmp_path), "trial_id": trial_id})
...
def test_mcp_wiki_query_pack_matches_cli_core_fields(tmp_path: Path):
    run_kactl(tmp_path, "wiki", "add-paper", "--paper-id", "P1", "--title", "Paper", "--summary", "Summary")
    run_kactl(tmp_path, "wiki", "add-idea", "--idea-id", "I1", "--title", "Idea", "--mechanism", "Mechanism")

    cli = run_kactl(tmp_path, "wiki", "query-pack", "--max-chars", "500")
    mcp = call_tool("ka_wiki_query_pack", {"project": str(tmp_path), "max_chars": 500})
...
def test_mcp_review_status_matches_cli_core_fields(tmp_path: Path):
    ...
    cli = run_kactl(tmp_path, "review", "status")
    mcp = call_tool("ka_review_status", {"project": str(tmp_path)})
...
def test_mcp_cost_status_matches_cli_core_fields(tmp_path: Path):
    ...
    cli = run_kactl(tmp_path, "cost", "status")
    mcp = call_tool("ka_cost_status", {"project": str(tmp_path), "daily_cap_usd": 10.0})
...
def test_mcp_runner_status_matches_cli_core_fields(tmp_path: Path):
    started = run_kactl(tmp_path, "runner", "start", "--command", "python train.py", "--dry-run")
    run_id = started["run"]["run_id"]

    cli = run_kactl(tmp_path, "runner", "status", run_id)
    mcp = call_tool("ka_runner_status", {"project": str(tmp_path), "run_id": run_id})
...
def test_mcp_queue_reconcile_matches_cli_core_fields(tmp_path: Path):
    run_kactl(tmp_path, "queue", "submit", "--job-id", "job1", "--command", "python train.py")
    run_kactl(tmp_path, "queue", "lease-next", "--worker-id", "w1", "--ttl-seconds", "0")

    cli = run_kactl(tmp_path, "queue", "reconcile")
    mcp = call_tool("ka_queue_reconcile", {"project": str(tmp_path)})
...
def test_mcp_soak_accelerated_matches_cli_core_fields(tmp_path: Path):
    cli = run_kactl(tmp_path, "soak", "accelerated", "--days", "10", "--inject-failures")
    mcp = call_tool("ka_soak_accelerated", {"project": str(tmp_path), "days": 10, "inject_failures": True})
...
def test_mcp_soak_crash_resume_matches_cli_core_fields(tmp_path: Path):
    ...
    cli = run_kactl(tmp_path, "soak", "crash-resume", "--restart-label", "mcp-test")
    mcp = call_tool("ka_soak_crash_resume", {"project": str(tmp_path), "restart_label": "mcp-test"})
