...
def test_migration_and_soak_cli_surface(tmp_path: Path):
    legacy = tmp_path / "Kvasir-agent" / "quests" / "legacy-001"
    legacy.mkdir(parents=True)
    (legacy / "quest.yaml").write_text("title: Legacy Quest\ngoal: Improve method\n", encoding="utf-8")

    migrated = run_kactl("migrate", "legacy-quests", "--format", "json", project_root=tmp_path)
    assert migrated["migrated_count"] == 1

    soak = run_kactl("soak", "accelerated", "--days", "10", "--inject-failures", "--format", "json", project_root=tmp_path)
