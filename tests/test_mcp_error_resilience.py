...
def test_missing_trial_and_runner_ids_return_not_found_not_traceback():
    trial = call_tool("ka_trial_show", {"trial_id": "T9999"})
    runner = call_tool("ka_runner_status", {"run_id": "R9999"})
...
def test_bad_mcp_arguments_return_recoverable_payload_not_traceback(tmp_path):
    payload = call_tool("ka_soak_accelerated", {"project": str(tmp_path), "days": "not-an-int"})
...
def test_tool_error_paths_redact_secret_like_input():
    ...
    bad_limit = call_tool("ka_queue_status", {"limit": secret})
    unknown = call_tool("ka_missing_" + secret, {})
