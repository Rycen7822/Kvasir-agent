...
def test_single_legacy_quest_is_conservatively_imported_on_first_write(tmp_path: Path):
    ...
    manifest = yaml.safe_load((tmp_path / "Kvasir-agent" / "research.yaml").read_text(encoding="utf-8"))
    assert manifest["quest"]["id"] == "QLEG"
    assert manifest["goal"]["title"] == "Legacy research"
    assert manifest["legacy"]["migrated_from"] == "Kvasir-agent/quests/QLEG"
    assert manifest["legacy"]["source_preserved"] is True
    assert (tmp_path / "Kvasir-agent" / "artifacts" / "metrics.json").read_text(encoding="utf-8") == '{"score": 1}\n'
    assert (tmp_path / "Kvasir-agent" / "runtime" / "goal_state.json").exists()
    assert (legacy_root / "quest.yaml").exists()

    report = json.loads((tmp_path / "Kvasir-agent" / "migrations" / "root_bound_migration_report.json").read_text(encoding="utf-8"))
    assert report["status"] == "imported"
    assert report["source"] == "Kvasir-agent/quests/QLEG"
...
def test_single_legacy_import_records_invalid_quest_id_mapping_in_report(tmp_path: Path):
    layout = ProjectLayout.from_project_root(tmp_path)
    legacy_root = tmp_path / "Kvasir-agent" / "quests" / "legacy-dir"
...
def test_multiple_legacy_quests_block_status_and_first_write(tmp_path: Path):
    ...
    status = call_tool("ka_status", {"project": str(tmp_path)})
    ...
    write = call_tool("ka_new_quest", {"project": str(tmp_path), "goal": "must block"})
...
def test_single_legacy_import_reports_conflicts_without_overwriting(tmp_path: Path):
    ...
    destination = tmp_path / "Kvasir-agent" / "artifacts" / "same_name.txt"
    ...
    assert not (tmp_path / "Kvasir-agent" / "research.yaml").exists()
    conflicts = json.loads((tmp_path / "Kvasir-agent" / "migrations" / "root_bound_conflicts.json").read_text(encoding="utf-8"))
