...
def run_toy_goal_research(project: Path) -> dict:
    quest = _ok(call_tool("ka_new_quest", {"project": str(project), "goal": "improve deterministic toy metric", "title": "Toy Goal E2E"}))
    quest_id = quest["quest"]["quest_id"]

    _ok(call_tool("ka_record_user_requirement", {"project": str(project), "quest_id": quest_id, "message": "Use only MCP tools and deterministic toy evidence.", "stage": "scout"}))
    baseline = _ok(call_tool("ka_create_local_baseline", {"project": str(project), "quest_id": quest_id, "baseline_id": "toy_baseline", "summary": "Deterministic score = 0.50", "overwrite": True}))
    _ok(call_tool("ka_confirm_baseline", {"project": str(project), **baseline["confirm_args"]}))
    ...
    runner = _ok(call_tool("ka_runner_start", {"project": str(project), "quest_id": quest_id, "job_id": "toy-e2e", "command": "python toy_train.py", "dry_run": True}))
    experiment = _ok(
        call_tool(
    ...
    scoreboard = _ok(call_tool("ka_update_method_scoreboard", {"project": str(project), "quest_id": quest_id, "idea_id": "toy-normalize", "outcome": "positive", "metric_delta": 0.12, "lesson": "normalization improved deterministic toy score", "mechanism": "normalize toy score before deterministic update"}))

    campaign = _ok(call_tool("ka_create_analysis_campaign", {"project": str(project), "quest_id": quest_id, "campaign_title": "Toy evidence analysis", "campaign_goal": "Check if toy improvement is claimable.", "slices": [{"slice_id": "toy-slice-1", "question": "Is the metric backed by evidence?"}]}))
    campaign_id = campaign.get("campaign_id") or campaign.get("campaign", {}).get("campaign_id") or "active"
    _ok(call_tool("ka_record_analysis_slice", {"project": str(project), "quest_id": quest_id, "campaign_id": campaign_id, "slice_id": "toy-slice-1", "status": "completed", "setup": "Toy analysis setup", "execution": "Review structured metric row", "results": "Evidence supports only a toy claim."}))

    evidence = project / "Kvasir-agent" / "artifacts" / "metrics" / "toy_metrics.json"
    evidence.parent.mkdir(parents=True, exist_ok=True)
    evidence.write_text(json.dumps({"primary": 0.62, "baseline": 0.50}) + "\n", encoding="utf-8")
    claim = _ok(call_tool("ka_claim_gate", {"project": str(project), "quest_id": quest_id, "claim_id": "toy-claim", "claim_text": "The toy normalization improves the deterministic toy score.", "baseline_id": "toy_baseline", "metric_contract": "primary", "evidence_paths": [str(evidence)], "analysis_slice_ids": ["toy-slice-1"], "seed_count": 3}))

    checkpoint = _ok(call_tool("ka_checkpoint", {"project": str(project), "quest_id": quest_id, "phase": "toy-e2e", "completed": ["baseline", "idea", "experiment", "method-ledger", "analysis", "claim-gate"], "decisions": ["claim limited to toy evidence"], "validation": ["MCP-only E2E completed"], "next_action": "continue with a real experiment or stop toy validation", "artifact_refs": [str(evidence)]}))
    resume = _ok(call_tool("ka_resume_brief", {"project": str(project), "quest_id": quest_id, "max_chars": 5000}))
    context_pack = _ok(call_tool("ka_context_pack", {"project": str(project), "quest_id": quest_id, "max_chars": 5000}))
