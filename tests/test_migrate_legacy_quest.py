...
def test_migrate_legacy_quest_creates_manifest_and_report_without_deleting_source(tmp_path: Path):
    legacy = tmp_path / "DeepScientist" / "quests" / "legacy-001"
    ...
    assert (tmp_path / "DeepScientist" / "research.yaml").exists()
    assert (legacy / "quest.yaml").exists(), "migration must not delete legacy source"
    report = json.loads((tmp_path / "DeepScientist" / "migrations" / "migration_report.json").read_text(encoding="utf-8"))
