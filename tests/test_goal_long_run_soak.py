...
def test_goal_long_run_soak_reaches_passive_checkpoint_without_cli_surface(tmp_path: Path):
    quest = _ok(call_tool("ka_new_quest", {"project": str(tmp_path), "goal": "toy long run", "title": "Toy Long Run"}))
    quest_id = quest["quest"]["quest_id"]
    _ok(call_tool("ka_record_user_requirement", {"project": str(tmp_path), "quest_id": quest_id, "message": "toy only"}))
    ...
    _ok(call_tool("ka_runner_start", {"project": str(tmp_path), "quest_id": quest_id, "command": "python train.py", "dry_run": True}))
    experiment = _ok(
        call_tool(
    ...
    _ok(call_tool("ka_update_method_scoreboard", {"project": str(tmp_path), "quest_id": quest_id, "idea_id": "toy", "outcome": "positive", "metric_delta": 0.1, "mechanism": "toy mechanism"}))
    checkpoint = _ok(call_tool("ka_checkpoint", {"project": str(tmp_path), "quest_id": quest_id, "phase": "soak", "completed": ["toy-loop"], "next_action": "analysis"}))
    resume = _ok(call_tool("ka_resume_brief", {"project": str(tmp_path), "quest_id": quest_id, "max_chars": 5000}))
