...
def test_manifest_queue_runner_trial_bridge_uses_mcp_service_handlers(tmp_path: Path):
    manifest = _assert_ok(call_tool("ka_manifest_init", {"project": str(tmp_path), "name": "bridge", "goal": "bridge goal"}))
    ...
    validation = _assert_ok(call_tool("ka_manifest_validate", {"project": str(tmp_path)}))
    assert validation["baseline_ready"] is True

    job = _assert_ok(call_tool("ka_queue_submit", {"project": str(tmp_path), "job_id": "job1", "command": "python train.py"}))
    assert job["job"]["job_id"] == "job1"
    assert job["job"]["status"] == "pending"

    run = _assert_ok(call_tool("ka_runner_start", {"project": str(tmp_path), "command": "python train.py", "job_id": "job1", "dry_run": True}))
    run_id = run["run"]["run_id"]
    status = _assert_ok(call_tool("ka_runner_status", {"project": str(tmp_path), "run_id": run_id}))
    ...
    planned = _assert_ok(call_tool("ka_trial_plan", {"project": str(tmp_path), "trial_id": trial_id, "metric_contract_id": "primary", "novelty_decision": "pass"}))
    assert planned["trial"]["status"] == "planned"
    ready = _assert_ok(call_tool("ka_trial_ready", {"project": str(tmp_path), "trial_id": trial_id}))
    assert ready["trial"]["status"] == "ready"
    evaluated = _assert_ok(call_tool("ka_trial_evaluate", {"project": str(tmp_path), "trial_id": trial_id, "metric_values": {"primary": 1.0}, "artifacts": []}))
    assert evaluated["trial"]["status"] == "evaluated"
    decided = _assert_ok(call_tool("ka_trial_decide", {"project": str(tmp_path), "trial_id": trial_id, "decision": "keep", "reviewer_verdict": "pass"}))
    assert decided["trial"]["status"] == "kept"


def test_mcp_fail_closed_without_cli_suggestion():
    payload = call_tool("ka_missing_research_primitive", {})
