...
def _run_kactl(project: Path, *args: str) -> dict:
...
def test_p3_log_and_artifact_tools_are_registered_in_explicit_goal_profile():
    ...
    assert {"ka_log_digest", "ka_artifact_index"} <= goal_names
    assert len(default_names) <= 14
    assert "ka_goal_state" not in default_names
    assert "ka_goal_next_action" not in default_names
    assert len(goal_names) < 48


def test_mcp_log_digest_and_artifact_index_match_cli_core_fields(tmp_path: Path):
    started = _run_kactl(tmp_path, "runner", "start", "--command", "python train.py", "--dry-run")
    run_id = started["run"]["run_id"]
    log_path = Path(started["run"]["log_path"])
    log_path.write_text("RuntimeError: failed token=" + "supersecret", encoding="utf-8")
    artifact_dir = tmp_path / "Kvasir-agent" / "artifacts"
    artifact_dir.mkdir(parents=True, exist_ok=True)
    artifact = artifact_dir / "result.txt"
    artifact.write_text("large artifact body", encoding="utf-8")

    cli_log = _run_kactl(tmp_path, "runner", "log-digest", run_id, "--max-tail-lines", "5")
    mcp_log = call_tool("ka_log_digest", {"project": str(tmp_path), "run_id": run_id, "max_tail_lines": 5})
    ...
    cli_artifacts = _run_kactl(tmp_path, "summary", "artifact-index", "--max-items", "10")
    mcp_artifacts = call_tool("ka_artifact_index", {"project": str(tmp_path), "max_items": 10})
