...
def test_review_claim_and_cost_cli_minimal_surface(tmp_path: Path):
    review = run_kactl(
        ...
        "--trial-id", "T0001",
        "--artifact-path", "Kvasir-agent/trials/T0001/metrics.json",
        "--verdict", "pass",
        "--notes", "ok",
        "--format", "json",
    ...
    cost = run_kactl("cost", "check", "--action-class", "GPU/cloud job", "--estimated-cost-usd", "2.5", "--daily-cap-usd", "1.0", "--format", "json", project_root=tmp_path)
