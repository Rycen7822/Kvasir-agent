...
def test_manifest_init_validate_show_cli_creates_project_local_research_yaml(tmp_path: Path):
    init_payload = run_kactl("manifest", "init", "--name", "Demo", "--goal", "Improve result", "--format", "json", project_root=tmp_path)
    ...
    validate_payload = run_kactl("manifest", "validate", "--format", "json", project_root=tmp_path)
    ...
    show_payload = run_kactl("manifest", "show", "--format", "json", project_root=tmp_path)
