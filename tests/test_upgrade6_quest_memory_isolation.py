...
def _new_quest(tmp_path: Path, title: str) -> str:
    payload = _ok(call_tool("ka_new_quest", {"project": str(tmp_path), "goal": title, "title": title}))
...
def test_memory_read_paths_without_state_fail_closed_and_never_fall_back_to_global(tmp_path: Path):
    calls = [
        ("ka_memory_search", {"project": str(tmp_path), "query": "global leak"}),
        ("ka_memory_list_recent", {"project": str(tmp_path)}),
        ("ka_memory_read", {"project": str(tmp_path), "card_id": "missing"}),
    ]
    ...
    assert not (tmp_path / "Kvasir-agent" / "home" / "memory").exists()
    assert not (tmp_path / "Kvasir-agent" / "memory").exists()
...
def test_root_bound_memory_uses_single_manifest_provenance_without_identity_switching(tmp_path: Path):
    ...
    same_research_search = _ok(call_tool("ka_memory_search", {"project": str(tmp_path), "quest_id": q2, "query": "unique-token-q1-only"}))
    assert same_research_search.get("quest_id") == q1
    assert same_research_search.get("matches")

    mismatch = call_tool("ka_memory_search", {"project": str(tmp_path), "quest_id": "different-quest", "query": "unique-token-q1-only"})
    ...
    for scope in ("global", "both"):
        payload = call_tool("ka_memory_search", {"project": str(tmp_path), "quest_id": q1, "query": "unique", "scope": scope})
