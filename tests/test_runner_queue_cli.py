...
def test_runner_and_queue_cli_expose_minimal_stable_control_surface(tmp_path: Path):
    started = run_kactl("runner", "start", "--command", "python train.py", "--dry-run", "--format", "json", project_root=tmp_path)
    assert started["run"]["run_id"] == "R0001"
    assert started["run"]["status"] == "dry_run"

    collected = run_kactl("runner", "collect", "R0001", "--exit-code", "1", "--format", "json", project_root=tmp_path)
    assert collected["run"]["status"] == "failed_other"
    assert collected["run"]["terminal"] is True

    submitted = run_kactl("queue", "submit", "--job-id", "job1", "--command", "python train.py", "--format", "json", project_root=tmp_path)
    assert submitted["job"]["status"] == "pending"

    updated = run_kactl("queue", "update", "job1", "--status", "completed", "--format", "json", project_root=tmp_path)
    assert updated["job"]["terminal"] is True

    status = run_kactl("queue", "status", "--format", "json", project_root=tmp_path)
