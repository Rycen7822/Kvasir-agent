...
def test_goal_watchdog_reports_stuck_runner_without_writing_goal_gate(tmp_path: Path):
    quest = _ok(call_tool("ka_new_quest", {"project": str(tmp_path), "goal": "crash resume", "title": "Crash Resume"}))
    ...
    watchdog = _ok(call_tool("ka_goal_watchdog", {"project": str(tmp_path), "quest_id": quest_id, "timeout_seconds": 0}))
    assert watchdog["stuck_runs"] == [run_id]
    assert watchdog["diagnostic"]["runner_stuck"] is True
    assert set(watchdog["diagnostic"]["recommended_evidence"]) == {"ka_log_digest", "ka_runner_status", "ka_queue_reconcile"}
    assert "current_gate" not in watchdog

    goal_state_path = tmp_path / "Kvasir-agent" / "runtime" / "goal_state.json"
    assert not goal_state_path.exists()

    resume = _ok(call_tool("ka_resume_brief", {"project": str(tmp_path), "quest_id": quest_id, "max_chars": 4000}))
