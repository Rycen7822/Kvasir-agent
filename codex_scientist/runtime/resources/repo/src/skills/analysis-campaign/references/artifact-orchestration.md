...
3. **Isolation setup**
   - call `artifact.submit_idea(...)` only for slices that need code or config divergence
...
Because `cs_artifact_record payload={...})` accepts extra fields, include:
