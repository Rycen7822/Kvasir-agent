...
Use when a CodexScientist quest-local quickstart file (for example `AGENTS.md`) contains stale state that conflicts with `cs_get_quest_state`, formal experiment command documents, or recently recorded decisions.
...
- CFC / ETD-score / RR-score / RawRepeat / Middle-cycle were clarified as experiment-internal comparators/controls, not formal CodexScientist baseline artifacts.
