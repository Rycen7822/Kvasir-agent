...
### CodexScientist code-only implementation variant

When the user asks to implement experiment code from an existing roadmap but explicitly says not to run experiments, use `references/codexscientist-code-only-implementation-passes.md`.
...
3. Order the document by the experiment phases (for CodexScientist, commonly E00-E12), starting with global shell variables and directory creation.
...
6. Include post-run validation commands and the exact information the user should provide back to CodexScientist for durable experiment recording.
