...
def render_main_experiment_metric_timeline_chart(
    *,
    ...
    normalized_style = str(style or "branded").strip().lower()
    if normalized_style != "branded":
...
def _draw_brand_lockup(draw: ImageDraw.ImageDraw, image: Image.Image, *, width: int) -> None:
    ...
    text_bbox = draw.textbbox((0, 0), "CodexScientist", font=brand_font)
    ...
    draw.text((text_left, 46), "CodexScientist", fill=MORANDI_TEXT, font=brand_font)
...
def _load_brand_mark() -> Image.Image | None:
    candidates = [
        ...
        Path(__file__).resolve().parents[3] / "assets" / "branding" / "codexscientist-mark.png",
    ]
