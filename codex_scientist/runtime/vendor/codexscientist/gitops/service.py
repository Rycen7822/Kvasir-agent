...
def _ensure_local_git_identity(repo: Path) -> None:
    """Ensure quest-local commits work even when global git identity is unset."""
    name = run_command(["git", "config", "--local", "user.name"], cwd=repo, check=False).stdout.strip()
    email = run_command(["git", "config", "--local", "user.email"], cwd=repo, check=False).stdout.strip()
    if not name:
        run_command(["git", "config", "--local", "user.name", "CodexScientist Agent"], cwd=repo, check=False)
    if not email:
        run_command(["git", "config", "--local", "user.email", "codexscientist@example.invalid"], cwd=repo, check=False)


def init_repo(repo: Path) -> None:
    run_command(["git", "init"], cwd=repo)
    run_command(["git", "branch", "-M", "main"], cwd=repo, check=False)
    _ensure_local_git_identity(repo)
...
def canonical_worktree_root(repo: Path, run_id: str) -> Path:
    return repo / ".cs" / "worktrees" / slugify(run_id, "run")
