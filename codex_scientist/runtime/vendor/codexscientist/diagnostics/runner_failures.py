    ...
    *,
    ...
    if "missing environment variable" in lower:
        return FailureDiagnosis(
            ...
            guidance=(
                "Set the required key in `~/CodexScientist/config/runners.yaml` under `runners.codex.env`.",
                "If you launch from a shell, export the provider key in that same shell before starting the runtime.",
            ),
            retriable=False,
    ...
    if any(marker in lower for marker in _MODEL_UNAVAILABLE_MARKERS):
        return FailureDiagnosis(
            ...
            guidance=(
                ...
                "If you need a fixed model, verify that the same model works in plain `codex exec` before retrying CodexScientist.",
            ),
            retriable=False,
