...
class PromptBuilder:
    ...
    def _active_communication_surface_block(
        ...
        *,
        ...
        return "\n".join(
            [
                "- latest_user_source: hermes",
                "- active_surface: hermes",
                "- active_connector: none",
                "- active_chat_type: hermes",
                "- active_chat_id: default",
                "- active_connector_origin: disabled_headless_backend",
                "- bound_external_connector_count: 0",
                "- surface_rule: CodexScientist is running as a Hermes-headless backend; Hermes is the only user-facing surface.",
                "- surface_reply_rule: write durable user-relevant progress with artifact.record(...); Hermes surfaces records/events to the user.",
                "- connector_contract_rule: built-in social connectors are disabled and must not be used.",
            ]
        )
    ...
    def _continuation_guard_block(
        ...
        *,
        ...
        lines = [
            ...
            "- early_stop_forbidden: do not stop, pause, or call a Hermes-controlled finalize recommendation record just because one turn, one stage, one run, or one checkpoint finished",
            "- completion_rule: only call a Hermes-controlled finalize recommendation record after a blocking completion approval request was sent and the user explicitly approved quest completion",
        ]
        ...
        if bash_running_count > 0:
            lines.extend(
                [
                    ...
                    "- long_run_watchdog_rule: while an important long-running bash_exec session is active, never let more than 30 minutes pass without inspecting real logs/status and sending a concise artifact.record progress update if the run is still ongoing",
                ]
            )
    ...
    def _next_required_step(self, *, snapshot: dict) -> str:
        ...
        if pending_user_count > 0:
            return f"Poll artifact.record(...) and handle the {pending_user_count} queued user message(s) first."
    ...
    def _recovery_resume_block(*, snapshot: dict, turn_reason: str) -> str:
        ...
        if not source.startswith("auto:runtime-recovery"):
            return "- none"
        lines = [
            ...
            "- recovery_rule: this turn exists because the runtime previously stopped or stale running state was reconciled; first re-establish the current truth before continuing any old stage loop.",
    ...
    def _interaction_style_block(self, *, default_locale: str, user_message: str, snapshot: dict) -> str:
        ...
        lines = [
            ...
            "- response_pattern: say what changed -> say what it means -> say what happens next",
            "- interaction_protocol: first message may be plain conversation; after that, treat artifact.record threads and mailbox polls as the main continuity spine across TUI, web, and connectors",
            ...
            "- mailbox_protocol: artifact.record(include_recent_inbound_messages=True) is the queued human-message mailbox; when it returns user text, treat that input as higher priority than background subtasks until it has been acknowledged",
            "- acknowledgment_protocol: after artifact.record returns any human message, immediately send one substantive artifact.record(...) follow-up; if the active connector runtime already emitted a transport-level receipt acknowledgement, do not send a redundant receipt-only message; if answerable, answer directly, otherwise state the short plan, nearest checkpoint, and that the current background subtask is paused",
            ...
            "- compaction_protocol: ordinary artifact.record progress updates should usually fit in 2 to 4 short sentences and should not read like a monitoring transcript or execution diary",
        ...
        if decision_policy == "autonomous":
            lines.extend(
                [
                    "- autonomous_decision_protocol: ordinary route choices belong to you; do not emit `artifact.record(payload={'kind': 'decision_request'}, ...)` for routine branching, baseline, cost, or experiment-selection ambiguity.",
