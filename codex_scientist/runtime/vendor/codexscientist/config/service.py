...
# Hermes-headless backend: connector packages are excluded from the packaged
# runtime. These inert helpers keep legacy config methods import-safe and
# disabled unless a future explicit connector plugin reintroduces them.
PROFILEABLE_CONNECTOR_NAMES: tuple[str, ...] = ()


def list_connector_profiles(*args: object, **kwargs: object) -> list[dict[str, object]]:
    return []


def normalize_connector_config(*args: object, **kwargs: object) -> dict[str, object]:
    return {}


def build_discovered_target(conversation_id: object, *args: object, **kwargs: object) -> dict[str, object]:
    return {"conversation_id": str(conversation_id or "")}


def infer_connector_transport(name: object, config: object | None = None) -> str:
    return "disabled"
...
def lingzhu_auth_ak_needs_rotation(value: object) -> bool:
    return False


def lingzhu_agent_id(config: dict | None = None) -> str | None:
    return None


def lingzhu_generated_curl(*args: object, **kwargs: object) -> str:
    return ""


def lingzhu_generated_openclaw_config_text(*args: object, **kwargs: object) -> str:
    return ""


def lingzhu_gateway_port(config: dict | None = None) -> int:
    return 0


def lingzhu_health_url(config: dict | None = None) -> str:
    return ""


def lingzhu_is_passive_conversation_id(*args: object, **kwargs: object) -> bool:
    return False


def lingzhu_local_base_url(config: dict | None = None) -> str:
    return ""


def lingzhu_passive_conversation_id(config: dict | None = None) -> str:
    return ""


def lingzhu_probe_payload(*args: object, **kwargs: object) -> dict[str, object]:
    return {}


def lingzhu_public_base_url(config: dict | None = None) -> str | None:
    return None


def lingzhu_sse_url(config: dict | None = None) -> str:
    return ""


def lingzhu_supported_commands(*args: object, **kwargs: object) -> list[str]:
    return []


def public_base_url_looks_public(value: object) -> bool:
    return False


def find_qq_profile(*args: object, **kwargs: object) -> dict[str, object] | None:
    return None


def list_qq_profiles(*args: object, **kwargs: object) -> list[dict[str, object]]:
    return []


def normalize_qq_connector_config(*args: object, **kwargs: object) -> dict[str, object]:
    return {}


def qq_profile_label(profile: dict | None = None) -> str:
    return "disabled"


def normalize_weixin_base_url(value: object) -> str:
    return str(value or "")


def normalize_weixin_cdn_base_url(value: object) -> str:
    return str(value or "")
...
class ConfigManager:
    ...
    def validate_named_text(self, name: str, content: str) -> dict:
        ...
        elif name == "native_tool_servers_disabled":
            native_tool_validation = self._validate_native_tool_servers_disabled_payload(normalized)
            warnings.extend(native_tool_validation["warnings"])
            errors.extend(native_tool_validation["errors"])
        return {
            "ok": len(errors) == 0,
            "warnings": warnings,
            "errors": errors,
...
This page edits `~/CodexScientist/config/connectors.yaml` directly.
...
- scan the QR code first so CodexScientist can persist `bot_token` and `account_id`
...
- the Hermes entry point records that user's `openid` into the profile `main_chat_id` when connector support is explicitly restored outside this native plugin
...
- Lingzhu is hosted directly by CodexScientist on `/metis/agent/api`
- keep `transport: openclaw_sse`
- use the same public CodexScientist origin and port that the browser is already serving on
- save once so CodexScientist can persist `auth_ak`; Rokid must use the same Bearer token
...
- the launcher runs a real Codex hello probe before first background runner start
- once Codex answers correctly, CodexScientist flips this flag to `true`
- if the probe fails, CodexScientist writes the failure summary back into config and blocks startup
...
- when you launch CodexScientist ad hoc with a provider profile, you can also use `cs --codex-profile <name>`
- when you want a one-off Codex binary override, you can also use `cs --codex /absolute/path/to/codex`
...
- CodexScientist hard-limits one turn to at most `5` total attempts, even if the config says more
...
"""
        if name == "native_tool_servers_disabled":
            return f"""# External native tool Guide

This page edits `{home_text}/config/native_tool_servers_disabled.yaml`.

## What belongs here

- external native tool server ids
...
1. add one server card per external native tool namespace
2. use `stdio` for local native tool processes
3. use `streamable_http` for remote native tool services
...
"""
        ...
        for name, raw_config in payload.items():
            ...
            if name == "telegram":
                ...
                if not str(config.get("local_host") or "").strip():
                    warnings.append("lingzhu: `local_host` is empty; CodexScientist will fall back to `127.0.0.1`.")
    ...
    def _test_single_connector(self, name: str, config: dict, *, live: bool, delivery_target: dict[str, object] | None = None) -> dict:
        ...
        if delivery_target and name != "lingzhu":
            ...
            if not delivery_requested:
                ...
                if name == "qq":
                    warnings.append(
                        "QQ readiness is healthy, but no OpenID has been learned yet. Save credentials, then send one private QQ message so CodexScientist can auto-detect and save the `openid`."
                    )
                else:
    ...
    def _default_connector_probe_message(self, connector_name: str) -> str:
        ...
        if locale.startswith("zh"):
            return f"老师您好，这是一条来自 CodexScientist 设置页的 {connector_name} 连接测试消息。若您收到这条消息，说明当前绑定与发送链路已经打通。"
        return (
            f"Hello — this is a CodexScientist {connector_name} settings test message. "
    ...
    def _prepare_codex_probe_home(
        ...
        *,
        ...
        temp_home = tempfile.TemporaryDirectory(prefix="cs-codex-probe-")
    ...
    def _codex_missing_binary_guidance(self, config: dict) -> list[str]:
        ...
        guidance = [
            "Run `npm install -g @researai/codexscientist` again so the bundled Codex dependency is installed.",
            "If `codex` is still missing, install it explicitly with `npm install -g @openai/codex`.",
        ]
        if profile:
            guidance.extend(
                [
                    f"Then verify `codex --profile {profile}` works from a terminal before starting CodexScientist.",
                    "If that profile uses a custom provider, make sure its API key and Base URL are configured in Codex first.",
                ]
            )
        else:
            guidance.append("Run `codex login` (or just `codex`) once and finish authentication before starting CodexScientist.")
        guidance.append(
            "If you use a custom Codex path, either set `runners.codex.binary` or launch with `cs --codex /absolute/path/to/codex`."
        )
        return guidance
    ...
    def _provider_profile_probe_hints(metadata: dict[str, object]) -> list[str]:
        ...
        if "coding.dashscope.aliyuncs.com" not in base_url:
            return [
                "Alibaba Bailian's generic DashScope / Qwen platform API is not supported by the Codex-backed CodexScientist path.",
    ...
    def _local_provider_probe_hints(metadata: dict[str, object]) -> list[str]:
        ...
        if requires_openai_auth is not False:
            hints.insert(
                0,
                "For local or self-hosted providers, add `requires_openai_auth = false` so CodexScientist can remove conflicting `OPENAI_*` auth variables.",
            )
        if not wire_api:
    ...
    def _missing_provider_env_guidance(
        *,
        ...
        guidance = [
            f"Set `runners.codex.env.{env_key}` in `~/CodexScientist/config/runners.yaml`, or export `{env_key}` before launching `cs`.",
        ]
        if provider_base_url_looks_local(str(metadata.get("base_url") or "").strip()):
            ...
            if metadata.get("requires_openai_auth") is not False:
                guidance.append(
                    "Also add `requires_openai_auth = false` to that local provider profile so CodexScientist can remove conflicting `OPENAI_*` auth variables."
                )
        guidance.append(
            f"Before retrying CodexScientist, run a real request such as `codex exec --profile {profile} --json --cd /tmp --skip-git-repo-check -` and verify it returns `HELLO`."
        )
        return guidance
    ...
    def _chat_wire_probe_version_block(
        ...
        *,
        ...
        errors = [
            "This provider uses `wire_api = \"chat\"`, but CodexScientist only probes chat-mode providers with `codex-cli 0.57.0`.",
        ]
        if detected_text:
            errors.append(f"Detected Codex CLI version: `{detected_text}`.")
        else:
            errors.append("CodexScientist could not determine the active Codex CLI version from the configured binary.")
        guidance = [
            "Install `npm install -g @openai/codex@0.57.0`, or point CodexScientist at a dedicated `0.57.0` binary with `cs --codex /absolute/path/to/codex`.",
        ...
        if profile:
            ...
            return (
                ...
                [
                    ...
                    "If the provider expects the model from the Codex profile itself, set `model: inherit` in `~/CodexScientist/config/runners.yaml`.",
                    *provider_hints,
                    *local_hints,
                    "Then run `cs doctor` and start CodexScientist again.",
                ],
            )
        return (
            [
                "Run `codex login` (or just `codex`) once and complete login before starting CodexScientist.",
            ],
            [
                ...
                "If the configured model is not available to your Codex account, update `~/CodexScientist/config/runners.yaml` and try again.",
                "Then run `cs doctor` and start CodexScientist again.",
            ],
        )
    ...
    def _probe_codex_runner(self, config: dict) -> dict:
        ...
        if not resolved_binary:
            return {
                ...
                "errors": [
                    ...
                    "CodexScientist could not resolve the bundled or configured `codex` CLI.",
                ],
                "details": details,
        ...
        if profile and effective_model == "inherit" and not self._codex_should_inherit_model(requested_model):
            compatibility_warnings.append(
                f"Codex profile `{profile}` is provider-backed. CodexScientist is probing it with `model: inherit`."
            )
            ...
            return {
                ...
                "guidance": [
                    *self._codex_probe_failure_guidance(config)[1],
                    ...
                    "Confirm the configured model is available to your Codex setup. CodexScientist currently probes Codex with the configured runner model first.",
                ],
            }
            ...
            if fallback_timeout is None and fallback_result is not None:
                ...
                if fallback_ok:
                    ...
                    fallback_warning = (
                        ...
                        "CodexScientist fell back to the current Codex default model."
                    )
                    return {
                        "ok": True,
                        ...
                        "details": details,
                        "guidance": [
                            "CodexScientist switched the Codex runner model to `inherit` so future runs keep using the current Codex default model.",
                        ],
                    }
            else:
        ...
        if not ok:
            ...
            if details.get("model_fallback_attempted") and not details.get("model_fallback_used"):
                warnings.append("CodexScientist also tried the current Codex default model, but that fallback probe did not succeed.")
        ...
        if not ok and missing_env_key and profile:
            errors.append(
                f"Codex profile `{profile}` requires environment variable `{missing_env_key}`, but CodexScientist did not receive it."
            )
            failure_guidance = [
    ...
    def _normalize_named_payload(self, name: str, payload: dict) -> dict:
        ...
        if name == "plugins":
            ...
            prepared = self._normalize_native_tool_payload(prepared)
    ...
    def _normalize_native_tool_payload(self, payload: dict) -> dict:
        normalized = deepcopy(payload)
    ...
    def _validate_native_tool_servers_disabled_payload(self, payload: dict) -> dict:
        warnings: list[str] = []
        errors: list[str] = []
        ...
        if not isinstance(servers, dict):
            return {
                "warnings": warnings,
                "errors": ["native_tool_servers_disabled: `servers` must be a mapping keyed by server id."],
            }

        for name, raw_server in servers.items():
            if not isinstance(raw_server, dict):
                errors.append(f"native_tool_servers_disabled: `{name}` must be a mapping.")
                continue
            ...
            if transport not in {"stdio", "streamable_http", "http", "sse"}:
                errors.append(
                    f"native_tool_servers_disabled: `{name}` transport `{transport}` is unsupported. Use `stdio`, `streamable_http`, `http`, or `sse`."
                )
            ...
            if enabled and transport == "stdio":
                ...
                if not any(str(item or "").strip() for item in command_list):
                    errors.append(f"native_tool_servers_disabled: `{name}` uses `stdio` but has no `command` configured.")
            if enabled and transport in {"streamable_http", "http", "sse"} and not url:
                errors.append(f"native_tool_servers_disabled: `{name}` uses `{transport}` but has no `url` configured.")
            if env is not None and not isinstance(env, dict):
                errors.append(f"native_tool_servers_disabled: `{name}` field `env` must be a mapping.")
        return {
            "warnings": warnings,
            "errors": errors,
        }
