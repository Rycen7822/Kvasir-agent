...
def default_config(home: Path) -> dict:
    return {
        ...
        "cloud": {
            ...
            "token": None,
            "token_env": "CODEXSCIENTIST_TOKEN",
            "verify_token_on_start": False,
            "sync_mode": "disabled",
        },
        "acp": {
            "compatibility_profile": "codexscientist-acp-compat/v1",
...
def default_runners() -> dict:
    return {
        "codex": {
            ...
            # Increase native csctl tool timeout so codex can wait for long `cs_bash_exec` await/read operations
            # or other durable native tool calls without prematurely timing out.
            # Mirrors CS_2027's `codex.native_tool_tool_timeout_sec` default.
