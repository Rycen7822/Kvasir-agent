...
def looks_like_codexscientist_root(path: Path) -> bool:
    if not path.exists() or not path.is_dir():
        return False
    if (path / "cli" / "bin" / "cs.js").exists():
        return True
...
def migrate_codexscientist_root(source: Path, target: Path) -> dict[str, Any]:
    ...
    if not looks_like_codexscientist_root(source):
        raise ValueError(f"Source path does not look like a CodexScientist home or install root: {source}")
    if source == target:
        raise ValueError("Source path and target path must be different.")
    if _is_relative_to(target, source):
        raise ValueError("Target path cannot be placed inside the current CodexScientist root.")
    if _is_relative_to(source, target):
        raise ValueError("Target path cannot be a parent of the current CodexScientist root.")
    if target.exists():
    ...
    try:
        ...
        return {
            ...
            "summary": "CodexScientist root copied and verified successfully.",
        }
    except Exception:
        shutil.rmtree(staging, ignore_errors=True)
        raise
