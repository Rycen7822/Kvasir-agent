...
class QuestStageViewBuilder:
    ...
    def _idea_stage_items(self) -> list[dict[str, Any]]:
        ...
        explicit_branch_selection = str(self.selection.get("selection_type") or "").strip() == "branch_node" and bool(
            str(self.selection.get("selection_ref") or self.selection.get("branch_name") or "").strip()
        )
        for item in self.artifacts:
            payload = self._payload(item)
            flow_type = str(payload.get("flow_type") or "").strip()
            ...
            if flow_type == "idea_submission" or kind == "idea":
                if self._branch_matches(payload, include_unscoped=not explicit_branch_selection):
                    items.append(item)
                    continue
                if explicit_branch_selection:
                    continue
    ...
    def _analysis_manifests(self) -> list[dict[str, Any]]:
        ...
        root = self.quest_root / ".cs" / "analysis_campaigns"
        if not root.exists():
            return manifests
