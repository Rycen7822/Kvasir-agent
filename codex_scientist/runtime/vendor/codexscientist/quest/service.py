...
class QuestService:
    ...
    def _binding_sources_payload(self, quest_root: Path) -> dict[str, list[str]]:
        bindings_path = quest_root / ".cs" / "bindings.json"
    ...
    def _runtime_state_lock_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "runtime_state.lock"
    ...
    def _research_state_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "research_state.json"

    @staticmethod
    def _lab_canvas_state_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "lab_canvas_state.json"
    ...
    def workspace_roots(self, quest_root: Path) -> list[Path]:
        ...
        worktrees_root = quest_root / ".cs" / "worktrees"
    ...
    def _artifact_projection_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "cache" / "artifact_projection.v2.json"

    @staticmethod
    def _artifact_projection_lock_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "artifact_projection.lock"

    @staticmethod
    def _metrics_timeline_cache_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "cache" / "metrics_timeline.v1.json"

    @staticmethod
    def _metrics_timeline_cache_lock_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "cache" / "metrics_timeline.lock"

    @staticmethod
    def _baseline_compare_cache_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "cache" / "baseline_compare.v1.json"

    @staticmethod
    def _baseline_compare_cache_lock_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "cache" / "baseline_compare.lock"
    ...
    def _projection_directory(quest_root: Path) -> Path:
        return quest_root / ".cs" / "projections"
    ...
    def _codex_history_events_state(self, quest_root: Path) -> tuple[tuple[str, tuple[int, int, int] | None], ...]:
        return self._glob_states(quest_root / ".cs" / "codex_history", "*/events.jsonl")
    ...
    def _details_projection_state(self, quest_root: Path) -> tuple[Any, ...]:
        ...
        core_paths = [
            ...
            quest_root / ".cs" / "runtime_state.json",
            quest_root / ".cs" / "research_state.json",
            quest_root / ".cs" / "interaction_state.json",
            quest_root / ".cs" / "bindings.json",
            quest_root / ".cs" / "bash_exec" / "summary.json",
    ...
    def _queue_projection_build(self, quest_root: Path, kind: str, *, source_signature: str) -> None:
        ...
        worker = threading.Thread(
            target=_worker,
            name=f"cs-projection-{quest_root.name}-{kind}",
        )
        with self._quest_projection_builds_lock:
    ...
    def _recent_codex_runs(self, quest_root: Path, *, limit: int = 5) -> list[dict[str, Any]]:
        history_root = quest_root / ".cs" / "codex_history"
        ...
        write_text(
            ...
            "\n".join(
                [
                    ...
                    "Existing runtime traces under `.cs/` were preserved.",
                    "",
                    "## Goal",
                    "",
                    restored_goal,
        ...
        write_text(
            ...
            "\n".join(
                [
                    "# Plan",
                    "",
                    "- [ ] Inspect preserved runtime traces under `.cs/`",
    ...
    def _codex_meta_state(self, quest_root: Path) -> tuple[tuple[str, tuple[int, int, int] | None], ...]:
        return self._glob_states(quest_root / ".cs" / "codex_history", "*/meta.json")
    ...
    def _snapshot_state(self, quest_root: Path) -> tuple[Any, ...]:
        core_paths = [
            ...
            quest_root / ".cs" / "runtime_state.json",
            quest_root / ".cs" / "research_state.json",
            quest_root / ".cs" / "user_message_queue.json",
            quest_root / ".cs" / "interaction_state.json",
            quest_root / ".cs" / "bindings.json",
            quest_root / ".cs" / "conversations" / "main.jsonl",
            quest_root / ".cs" / "bash_exec" / "summary.json",
        ]
        return (
            self._path_states(core_paths),
    ...
    def _snapshot(self, quest_id: str) -> dict:
        ...
        history = self._read_cached_jsonl(quest_root / ".cs" / "conversations" / "main.jsonl")
        artifacts = []
        recent_runs = []
        ...
        codex_history_root = quest_root / ".cs" / "codex_history"
        ...
        paths = {
            ...
            "bash_exec_root": str(quest_root / ".cs" / "bash_exec"),
        }
        counts = {
    ...
    def append_message(
        ...
        *,
        ...
        interaction_state_path = quest_root / ".cs" / "interaction_state.json"
        interaction_state = self._read_interaction_state(quest_root)
        ...
        append_jsonl(quest_root / ".cs" / "conversations" / "main.jsonl", record)
        if role == "user":
            recent_threads = [dict(item) for item in (interaction_state.get("recent_threads") or [])]
        ...
        if role == "user":
            ...
            if status == "waiting_for_user":
                interaction_state = read_json(quest_root / ".cs" / "interaction_state.json", {"open_requests": []})
    ...
    def history(self, quest_id: str, limit: int = 100) -> list[dict]:
        return self._read_cached_jsonl(self._quest_root(quest_id) / ".cs" / "conversations" / "main.jsonl")[-limit:]
    ...
    def events(
        ...
        *,
        ...
        event_path = self._quest_root(quest_id) / ".cs" / "events.jsonl"
    ...
    def _read_interaction_state(self, quest_root: Path) -> dict[str, Any]:
        state = self._read_cached_json(quest_root / ".cs" / "interaction_state.json", {})
    ...
    def _runtime_state_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "runtime_state.json"

    @staticmethod
    def _agent_status_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "agent_status.json"

    @staticmethod
    def _message_queue_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "user_message_queue.json"

    @staticmethod
    def _interaction_journal_path(quest_root: Path) -> Path:
        return quest_root / ".cs" / "interaction_journal.jsonl"
    ...
    def _write_active_user_requirements(
        ...
        *,
        ...
        user_messages = [
            item
            for item in read_jsonl(quest_root / ".cs" / "conversations" / "main.jsonl")
    ...
    def record_artifact_interaction(
        ...
        *,
        ...
        runtime_updates: dict[str, Any] = {
            ...
            "last_tool_activity_name": "cs_artifact_record",
            "tool_calls_since_last_artifact_interact": 0,
            "pending_user_message_count": len((self._read_message_queue(quest_root).get("pending") or [])),
        }
        ...
        *,
        ...
        if delivered_messages:
            lines = [
                ...
                self.localized_copy(
                    quest_root=quest_root,
                    zh="- 完成该用户请求后，再立刻通过 Hermes 汇报完整结果，并按需使用 cs_artifact_record 记录进展。",
                    en="- After completing that user request, immediately report the full result through Hermes and use cs_artifact_record when a durable progress artifact is needed.",
                ),
                self.localized_copy(
                    quest_root=quest_root,
    ...
    def _snapshot_folder_kind(tree: dict[str, dict | None], relative: str) -> str | None:
        ...
        if not normalized or normalized.startswith(".cs/"):
            return None
    ...
    def _skip_explorer_relative(relative: str) -> bool:
        if relative.startswith(".git/") or relative == ".git":
            return True
        if relative.startswith(".cs/worktrees/"):
            return True
        parts = PurePosixPath(relative).parts
    ...
    def _skip_explorer_profile_relative(relative: str, profile: str | None) -> bool:
        ...
        if top in {".codex", ".claude", ".cs", "tmp", "userfiles", "artifacts"}:
            return True
    ...
    def _classify_relative_scope(relative: str) -> tuple[str, bool]:
        ...
        if relative.startswith(".cs/codex_history/"):
            return "runner_history", False
        if relative.startswith(".cs/"):
