...
_COMPAT_BEGIN_MARKER = "# BEGIN CODEXSCIENTIST PROFILE COMPAT"
_COMPAT_END_MARKER = "# END CODEXSCIENTIST PROFILE COMPAT"
    ...
    *,
    ...
    return (
        ...
        (
            ...
            "CodexScientist downgraded reasoning effort to `high` automatically."
        ),
    )
...
def adapt_profile_only_provider_config(
    ...
    *,
    ...
    return (
        adapted,
        (
            f"CodexScientist overrode conflicting top-level {field_text} with values from profile "
            f"`{normalized_profile}` for Codex compatibility."
            if conflicted_fields
            else f"CodexScientist promoted `{normalized_profile}` profile {field_text} to the top level for Codex compatibility."
        ),
    )
