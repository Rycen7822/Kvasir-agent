#!/usr/bin/env python3
"""CodexScientist native control script.

This is the primary native integration surface for Codex CLI. It imports the
CodexScientist runtime directly and invokes curated cs_* handlers. It is
not an MCP server and it never calls the external legacy `cs` command for normal
operation.
"""
...
def _friendly_payload(tool_name: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if tool_name == "cs_new_quest" and "quest_id" not in payload:
    ...
    if tool_name == "cs_artifact_record":
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="CodexScientist native CLI fallback. MCP is provided by cs_mcp.py.")
    parser.add_argument("--project-root", help="Project root whose ./CodexScientist runtime should be used. Defaults to cwd or CODEXSCIENTIST_PROJECT_ROOT.")
    parser.add_argument("--format", choices=["json", "pretty"], default="pretty")
    sub = parser.add_subparsers(dest="command")

    p = sub.add_parser("list-tools", help="List native cs_* tools exposed to Codex")
    ...
    p.add_argument("--format", choices=["json", "pretty"], default=None)
    p.set_defaults(func=lambda ns: call_tool("cs_doctor", {}))

    p = sub.add_parser("call", help="Call a native cs_* tool")
    ...
    for tool_name in ["cs_doctor", "cs_list_quests", "cs_get_quest_state", "cs_events", "cs_new_quest", "cs_set_active_quest", "cs_memory_search", "cs_memory_write", "cs_artifact_record", "cs_bash_exec"]:
