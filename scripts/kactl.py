#!/usr/bin/env python3
"""Kvasir-agent CLI compatibility entrypoint."""
...
import ka_native_cli  # type: ignore  # noqa: E402
from kvasir_agent.adapters.cli import normalize_envelope  # noqa: E402
from kvasir_agent.services.artifacts import ArtifactIndexService  # noqa: E402
from kvasir_agent.services.checkpoint import CheckpointService  # noqa: E402
from kvasir_agent.services.claims import ClaimEvidenceService  # noqa: E402
from kvasir_agent.services.context_pack import ContextPackService  # noqa: E402
from kvasir_agent.services.costs import CostApprovalService  # noqa: E402
from kvasir_agent.services.frontier import FrontierService  # noqa: E402
from kvasir_agent.services.journal import JournalService  # noqa: E402
from kvasir_agent.services.manifest import ManifestService  # noqa: E402
from kvasir_agent.services.migrations import MigrationService  # noqa: E402
from kvasir_agent.services.project_state import ProjectLayout  # noqa: E402
from kvasir_agent.services.queue import QueueService  # noqa: E402
from kvasir_agent.services.research_wiki import ResearchWikiService  # noqa: E402
from kvasir_agent.services.review import ReviewService  # noqa: E402
from kvasir_agent.services.resume import ResumeService  # noqa: E402
from kvasir_agent.services.runner import RunnerService  # noqa: E402
from kvasir_agent.services.soak import SoakService  # noqa: E402
from kvasir_agent.services.trial import TrialService  # noqa: E402
...
def build_ka_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Kvasir-agent native kactl. MCP is available through ka_mcp.py; this remains the CLI fallback.")
    parser.add_argument("--project-root", help="Project root whose ./Kvasir-agent runtime should be used. Defaults to cwd.")
    parser.add_argument("--format", choices=["json", "pretty"], default="pretty")
    sub = parser.add_subparsers(dest="command")

    manifest = sub.add_parser("manifest", help="Manage project-local Kvasir-agent/research.yaml")
    manifest_sub = manifest.add_subparsers(dest="manifest_command", required=True)
    init = manifest_sub.add_parser("init", help="Create Kvasir-agent/research.yaml")
    init.add_argument("--name", required=True)
    init.add_argument("--goal", required=True)
    init.add_argument("--overwrite", action="store_true")
    _add_format(init)
    init.set_defaults(func=_manifest_payload)
    validate = manifest_sub.add_parser("validate", help="Validate Kvasir-agent/research.yaml")
    _add_format(validate)
    validate.set_defaults(func=_manifest_payload)
    show = manifest_sub.add_parser("show", help="Show Kvasir-agent/research.yaml")
    _add_format(show)
    ...
    context_pack = summary_sub.add_parser("context-pack", help="Write bounded Kvasir-agent/summaries/context_pack.md")
    ...
    migrate = sub.add_parser("migrate", help="Migrate legacy Kvasir-agent state into the project-local control plane")
    migrate_sub = migrate.add_subparsers(dest="migrate_command", required=True)
    legacy_quests = migrate_sub.add_parser("legacy-quests", help="Migrate legacy Kvasir-agent/quests metadata without deleting sources")
...
def _looks_like_ka_command(argv: list[str]) -> bool:
...
def main(argv: list[str] | None = None) -> int:
    raw_argv = ka_native_cli._normalize_legacy_cli_args(list(sys.argv[1:] if argv is None else argv))
    if _looks_like_ka_command(raw_argv):
        parser = build_ka_parser()
        ...
        ka_native_cli.emit(payload, fmt)
        return 0 if payload.get("ok", False) else 1

    parser = ka_native_cli.build_parser()
    args = parser.parse_args(raw_argv)
